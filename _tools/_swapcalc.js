...
+'.lxo-o .m{flex:1 1 auto;min-width:0;display:flex;flex-direction:column;gap:7px}'
// three labelled lines: what leaves, what arrives, at what rate. The label column is fixed so the three
// values stack in a straight edge and the row reads down rather than across.
+'.lxo-r{display:flex;align-items:center;gap:10px;min-width:0}'
+'.lxo-k{flex:0 0 52px;font-size:12px;color:var(--text-muted)}'
+'.lxo-leg{display:inline-flex;align-items:center;gap:8px;min-width:0;font-size:14.5px;font-weight:700;color:var(--text)}'
+'.lxo-leg .q{font-family:\'JetBrains Mono\',monospace}'
+'.lxo-leg .c{color:var(--text-soft);font-weight:700}'
+'.lxo-leg .lxq-ic{width:22px;height:22px;border-radius:50%;background:linear-gradient(135deg,#2a2a35,#1a1a23);color:#fff;font-size:10px;font-weight:800;display:inline-flex;align-items:center;justify-content:center;flex:0 0 auto}'
+'.lxo-px{font-size:12.5px;color:var(--text-soft);font-family:\'JetBrains Mono\',monospace;min-width:0;overflow:hidden;text-overflow:ellipsis}'
...
+'function qLeg(a,amount){'
+'var w=document.createElement("div");w.className="lxo-leg";'
+'var ic=document.createElement("span");ic.className="lxq-ic";ic.setAttribute("data-lxc",a.code||"");'
+'ic.textContent=(a.code||"?").slice(0,2);'
+'var qty=document.createElement("span");qty.className="q";qty.textContent=qNum(amount);'
+'var cd=document.createElement("span");cd.className="c";cd.textContent=a.code||"";'
+'w.appendChild(ic);w.appendChild(qty);w.appendChild(cd);'
+'try{ qPaintIco(ic,a); }catch(_){}'
+'return w;}'
+'function qOrdRow(label,node){'
+'var r=document.createElement("div");r.className="lxo-r";'
+'var l=document.createElement("span");l.className="lxo-k";l.textContent=label;'
...
+'m.appendChild(qOrdRow("Selling",qLeg(s,amt)));'
+'m.appendChild(qOrdRow("For",qLeg(b,amt*pr)));'
+'var pn=document.createElement("span");pn.className="lxo-px";'
+'pn.textContent=qNum(pr)+" "+b.code+" per "+s.code;'
+'m.appendChild(qOrdRow("Price",pn));'
+'row.appendChild(m);'
...
+'[[s,m.children[0]],[b,m.children[1]]].forEach(function(pair){'
...
+'cb.textContent="Cancel";row.appendChild(cb);'
