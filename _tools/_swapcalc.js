...
// RAZA 2026-09-17: "why does it come prefilled with 0.00 value. everytime i wanna enter, i first need to remove
// 0.00". You receive is a contenteditable div, and its empty state was the literal TEXT "0.00" -- so typing into it
// meant selecting and deleting first, every time. It is now genuinely empty and draws the 0.00 in CSS, which is what
// the You pay input beside it has always done with a placeholder attribute. user-select:none keeps a Select-All from
// picking the ghost up, and it is never in the DOM, so every parseFloat of this field still reads what was typed.
+'#swapAmtOut:empty::before{content:"0.00";opacity:.5;pointer-events:none;user-select:none;-webkit-user-select:none}'
...
// The input inside carries its own focus ring; a second one around the whole block is just noise.
+'#modalSwap .swap-pair .field:focus-within{border-color:rgba(127,127,140,.28)!important;box-shadow:none!important}'
// Keep the LUMOS mention in the fee banner a text link, whatever the logo painter tries.
+'.lx-fee-buy{background:none!important;background-image:none!important;border-radius:0!important;'
+'width:auto!important;height:auto!important;min-width:0!important;display:inline!important;'
+'box-shadow:none!important;padding:0!important;font-size:inherit!important;'
+'color:var(--accent,#ea6a2c)!important;text-decoration:none;border-bottom:1px solid rgba(234,106,44,.35)}'
+'.lx-fee-buy:hover{border-bottom-color:var(--accent,#ea6a2c)}'
+'.lx-fee-buy svg{width:0!important;height:0!important;position:absolute!important}'
...
+'return fetch("/lxapi/assetsearch?search="+encodeURIComponent(a.issuer)+"&limit=20")'
...
+'fetch("/lxapi/assetsearch?search="+encodeURIComponent(v)+"&limit=12")'
...
+'var _plx=tb.setTimeout(300).build().toXDR(),_plp=window.lxSign(_plx,S);'
...
+'var nat=(b.asset_type==="native");return{code:nat?"XLM":b.asset_code,iss:nat?"":(b.asset_issuer||""),bal:(nat?(+b.balance||0):Math.max(0,(+b.balance||0)-(+b.selling_liabilities||0))),total:(+b.balance||0),native:nat};'
...
+'var k=a.code+"|"+a.issuer;'
+'if(QUSD["art:"+k]!==undefined){ if(QUSD["art:"+k]){a.img=QUSD["art:"+k];qPaintIco(el,a);} return; }'
+'fetch("/lxapi/assetsearch?search="+encodeURIComponent(a.issuer)+"&limit=20")'
+'.then(function(r){return r.json();}).then(function(dd){'
+'var rr=((dd&&dd._embedded&&dd._embedded.records)||[]).filter(function(x){'
+'return String(x.asset||"").indexOf(a.code+"-"+a.issuer)===0;})[0];'
...
+'var _cnx=tb.setTimeout(300).build().toXDR(),_cnp=window.lxSign(_cnx,S);'
// Bare lxSign never settles when the wallet does not answer, and an unsettled promise cannot reach
// the .catch below -- which is why this button used to stick on "Confirm..." for good. 200s matches
// every other signing site and outlasts the 180s SEP-7 poll.
...
// Top level on purpose: both confirm handlers read the panel's exact values through this, and they live
// in bootA/bootB. It used to sit inside calc(), where neither could reach it.
...
+'var tx=tb2.setTimeout(300).build();'
...
+'function dashLogo(a){if(!a)return "";if(a.native||a.code==="XLM")return window.__lxStellarUri||window.__lxDSTU||"";return a.logo||(window.__lxLogos||{})[a.code]||(window.lxLogoFor?window.lxLogoFor(a.code,a.iss||""):"")||"";}'
...
+'function dis(b){if(!go)return;if(b){go.setAttribute("data-lxdis","1");go.style.opacity="0.55";go.style.cursor="not-allowed";}'
+'else{go.removeAttribute("data-lxdis");go.style.opacity="";go.style.cursor="";}}'
...
+'function pick(which,a){if(a.iss&&a.code!=="XLM"){window.__lxKnownSwap=window.__lxKnownSwap||{};window.__lxKnownSwap[a.code]=a.iss;}if(!a.native&&a.bal==null){var hb=dashHeld(a.code);if(hb)a.bal=hb;}if(which==="from")fromA=a;else toA=a;if(fromA.code===toA.code){var d=(a.code==="XLM")?{code:"USDC",iss:lxIssuer("USDC"),native:false,bal:dashHeld("USDC")||null}:{code:"XLM",iss:"",native:true,bal:null};if(which==="from")toA=d;else fromA=d;}applyChips();inp.value&&inp.value.trim()?run(false):(out&&(out.textContent=""),panel.style.display="none");spotRate=null;quoteRate();}'
+'window.lxSwapPrefill=function(o){try{if(!o)return false;'+'if(o.from)pick("from",o.from); if(o.to)pick("to",o.to);'+'if(o.amount!=null&&o.amount!==""){inp.value=String(o.amount);'+'try{inp.dispatchEvent(new Event("input",{bubbles:true}));}catch(_){run(false);}}'+'applyChips();return true;}catch(_){return false;}};'
...
+'function run(preview){var raw=(inp.value||"");if(/[^0-9.,\\s]/.test(raw)){showErr("Enter a valid amount (numbers only)");dis(true);if(out)out.textContent="";panel.style.display="none";useSoro=null;return;}var amt=parseFloat(raw.replace(/,/g,""))||0;var fa=fromA,ta=toA,fr=(window.__lxFeeRate||0.002),net=amt*(1-fr);if(!(amt>0)){if(out)out.textContent="";showErr("");dis(true);panel.style.display="none";useSoro=null;return;}'
...
// The design ships this div with 0.00 already inside it, so the first open would still have shown a value to delete.
+'if(!/[1-9]/.test(out.textContent||""))out.innerHTML="";'
...
+'if(!(out.textContent||"").trim()&&out.innerHTML!=="")out.innerHTML="";'
+'clearTimeout(_rvT);'
...
+'function resetFields(){inp.value="";if(out)out.textContent="";showErr("");panel.style.display="none";useSoro=null;dis(true);if(!window.__lxHoldings)dashBoot();spotRate=null;quoteRate();}'
...
// LOGOS FOR EVERY ASSET, not only the ones something else happened to cache (RAZA 2026-09-19: AQUA, BLND, ARMYXLM
// showed letter discs in this menu while LUMOS had its mark). After the cache: the curated logo shipped with the site
// (assets/tokens/curated, fetched by _bridgelogos.js), then our logo endpoint by CODE-ISSUER -- a ticker is not an
// identity on Stellar, so the issuer is always part of the key. A failed image falls back to the letter (see below).
+'var LX_CURLOGO=' + JSON.stringify((function(){ try{ return require('fs').readdirSync(require('path').join(__dirname,'..','assets','tokens','curated')).filter(function(f){ return /\.png$/.test(f); }).map(function(f){ return f.replace(/\.png$/,''); }); }catch(_){ return []; } })()) + ';'
+'var LX_LOCLOGO={USDC:"/assets/tokens/usdc.png",SHX:"/assets/tokens/shx.png",yXLM:"/assets/tokens/yxlm.png",LUMOS:"/assets/favicon.png",BLND:"/assets/tokens/blnd.svg",AQUA:"/assets/tokens/aqua.png",USDT0:"/assets/tokens/usdt0.png"};'
+'var LX_LOCISS={USDC:"GA5ZSEJYB37JRC5AVCIA5MOP4RHTM335X2KGX3IHOJAPP5RE34K4KZVN",SHX:"GDSTRSHXHGJ7ZIVRBXEYE5Q74XUVCUSEKEBR7UCHEUUEK72N7I7KJ6JH",yXLM:"GARDNV3Q7YGT4AKSDF25LT32YSCCW4EV22Y2TV3I2PU2MMXJTEDL5T55",LUMOS:"GB5T2EQC2VDG2XEYQ5C2CQJ2SCB5RFPPWALUU2GQ3R5HUEGOZST55B6S",BLND:"GDJEHTBE6ZHUXSWFI642DCGLUOECLHPF3KSXHPXTSTJ7E3JF6MQ5EZYY",AQUA:"GBNZILSTVQZ4R7IKQDGHYGY2QXL5QOFJYQMXPKWRRM5PAV7Y4M67AQUA",USDT0:"GATISXX6BZ6NC7IKQBY37CJD4SOZL3CYZJWXEDG6JVIY4WBS6KXJHN6Q"};'
// THE CURATED LIST, for the menus' default lists (RAZA 2026-09-19: LMNR sat in the Custom Swap menu -- "we haven't
// curated it"; xLMNR is the curated one). Same source as the bridge's From list: lib.js VERIFIED + GENERATED_ASSETS,
// keyed CODE|ISSUER, because a ticker is not an identity (LMNR and xLMNR, or any fake USDC, differ by issuer).
+'var LX_CURSET=' + JSON.stringify((function(){ try{ var L=require(__dirname+'/lib.js'); return Object.keys(L.VERIFIED).filter(function(k){ return k.split('|')[1]; }).concat((L.GENERATED_ASSETS||[]).map(function(a){ return a.code+'|'+a.issuer; })).filter(function(k,i,a){ return a.indexOf(k)===i; }); }catch(_){ return []; } })()) + ';'
+'window.lxIsCurated=function(code,iss){return LX_CURSET.indexOf(code+"|"+(iss||""))>=0;};'
// ONE lookup for both menus on the page (this one and the Custom Swap popup's dashLogo), on window so they cannot drift
+'window.lxLogoFor=function(code,iss){if(!code||!/^G[A-Z2-7]{55}$/.test(iss||""))return "";'
+'if(LX_LOCISS[code]===iss)return LX_LOCLOGO[code];'
+'if(LX_CURLOGO.indexOf(code+"-"+iss)>=0)return "/assets/tokens/curated/"+code+"-"+iss+".png";'
+'return "/lxapi/logoimg?asset="+encodeURIComponent(code+"-"+iss);};'
+'function swLogo(h){if(h.native)return window.__lxStellarUri||"";var c=(window.__lxLogos||{})[h.code];if(h.logo||c)return h.logo||c;'
+'return window.lxLogoFor(h.code,h.iss||(typeof lxIssuer==="function"?lxIssuer(h.code):"")||"");}'
// an image that does not load becomes the letter disc again, rather than an empty circle
+'document.addEventListener("error",function(e){var t=e.target;if(!t||t.tagName!=="IMG"||!t.parentNode||!t.parentNode.classList||!t.parentNode.classList.contains("lx-am-ic"))return;var p=t.parentNode,cd=t.getAttribute("data-code")||"?";p.style.background=swCol(cd);p.textContent=cd.slice(0,1).toUpperCase();},true);'
...
+'function swIcoInner(h){var lg=swLogo(h);if(lg)return \'<img src="\'+lg+\'" data-code="\'+esc(h.code||"?")+\'" alt="" style="width:100%;height:100%;object-fit:cover;display:block">\';return h.native?"\\u2726":esc((h.code||"?").slice(0,1).toUpperCase());}'
