...
+'.lx-fee-buy{flex:0 0 auto;background:var(--accent,#ea6a2c);color:#fff!important;font-weight:700;font-size:11.5px;letter-spacing:.01em;padding:8px 13px;border-radius:9px;text-decoration:none;white-space:nowrap;transition:filter .12s}'
...
+'.lx-swap-menu .lx-am-list{flex:1 1 auto;overflow-y:auto;min-height:0;max-height:210px}'
...
// --- Limit tab -------------------------------------------------------------------------------------
+'.lxq-f{display:flex;flex-direction:column;gap:9px;position:relative}'
+'.lxq-row{display:flex;align-items:center;gap:10px;background:var(--surface-2);border:1px solid var(--border);border-radius:11px;padding:10px 12px}'
+'.lxq-row label{flex:0 0 62px;font-size:11.5px;font-weight:700;color:var(--text-soft);letter-spacing:.02em}'
+'.lxq-pick{flex:1 1 auto;min-width:0;display:flex;align-items:baseline;gap:7px;cursor:pointer;font-weight:700;font-size:13.5px;color:var(--text)}'
+'.lxq-pick .g{font-size:10.5px;color:var(--text-muted);font-weight:500;font-family:\'JetBrains Mono\',monospace}'
+'.lxq-pick .cv{opacity:.55;font-size:10px}'
+'.lxq-row input{flex:1 1 80px;min-width:0;background:transparent;border:0;outline:none;font:inherit;font-size:14px;font-weight:700;color:var(--text);text-align:right}'
+'.lxq-unit{font-size:11px;color:var(--text-muted);white-space:nowrap}'
+'.lxq-bal{display:flex;justify-content:space-between;align-items:center;font-size:11.5px;color:var(--text-muted);margin:-3px 3px 0}'
+'.lxq-bal button{background:none;border:0;color:var(--accent);font:inherit;font-size:11.5px;font-weight:700;cursor:pointer;padding:0}'
+'.lxq-tot{display:flex;justify-content:space-between;font-size:13px;font-weight:700;padding:1px 3px;color:var(--text-soft)}'
+'.lxq-tot b{color:var(--text)}'
+'.lxq-note{font-size:11.5px;line-height:1.45;color:var(--text-muted);margin:0 3px}'
+'.lxq-err{color:var(--red,#ff5b5b);font-size:12px;font-weight:600;margin:0 3px}'
+'.lxq-go{width:100%;padding:12px;border:0;border-radius:11px;background:linear-gradient(180deg,#ff8a4c,var(--accent,#ea6a2c));color:#fff;font:inherit;font-size:14px;font-weight:800;cursor:pointer}'
+'.lxq-go[disabled]{opacity:.5;cursor:not-allowed}'
+'.lxq-menu{position:absolute;z-index:60;left:12px;right:12px;background:var(--surface);border:1px solid var(--border);border-radius:11px;padding:5px;box-shadow:0 20px 44px -20px rgba(0,0,0,.6);max-height:250px;overflow:auto}'
+'.lxq-menu button{display:flex;flex-direction:column;align-items:flex-start;gap:1px;width:100%;padding:7px 9px;border:0;border-radius:8px;background:transparent;color:var(--text);font:inherit;font-size:13px;font-weight:700;text-align:left;cursor:pointer}'
+'.lxq-menu button:hover{background:var(--surface-2)}'
+'.lxq-menu button .s{font-size:10.5px;font-weight:500;color:var(--text-muted);font-family:\'JetBrains Mono\',monospace}'
+'.lxq-menu input{width:100%;padding:7px 9px;margin-bottom:4px;background:var(--surface-2);border:1px solid var(--border);border-radius:8px;font:inherit;font-size:12.5px;color:var(--text);outline:none}'
+'.lxq-menu .h{padding:6px 9px 3px;font-size:10.5px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:var(--text-muted)}'
// --- Orders tab ------------------------------------------------------------------------------------
+'.lxo-list{display:flex;flex-direction:column;gap:8px;min-height:64px}'
+'.lxo-o{display:flex;align-items:center;gap:10px;background:var(--surface-2);border:1px solid var(--border);border-radius:11px;padding:10px 12px}'
+'.lxo-o .m{flex:1 1 auto;min-width:0}'
+'.lxo-o .p{font-size:13px;font-weight:700;color:var(--text)}'
+'.lxo-o .s{font-size:11px;color:var(--text-muted);margin-top:2px}'
+'.lxo-o button{flex:0 0 auto;background:none;border:1px solid var(--border);border-radius:8px;color:var(--text-soft);font:inherit;font-size:11.5px;font-weight:700;padding:6px 11px;cursor:pointer}'
+'.lxo-o button:hover:not([disabled]){color:var(--red,#ff5b5b);border-color:var(--red,#ff5b5b)}'
+'.lxo-o button[disabled]{opacity:.55;cursor:default}'
+'.lxo-empty{padding:24px 8px;text-align:center;color:var(--text-muted);font-size:12.5px;line-height:1.5}'
+'</style>';
...
+'function qHold(){return (window.__lxHoldings||[]).map(function(b){'
+'return b.asset_type==="native"?{code:"XLM",issuer:"",native:true,bal:+b.balance}'
...
+'var top=anchor.offsetTop+anchor.offsetHeight+6;m.style.top=top+"px";'
+'function row(a,sub){var b=document.createElement("button");b.type="button";'
+'b.innerHTML="<span>"+a.code+"</span><span class=\\"s\\">"+(sub||(a.native?"native":qShort(a.issuer)))+"</span>";'
+'b.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();'
+'if(side==="sell")qS=a;else qB=a;qClose();qSync();},true);m.appendChild(b);}'
+'var hd=document.createElement("div");hd.className="h";hd.textContent="Your assets";m.appendChild(hd);'
+'qHold().forEach(function(a){ if(a.bal>0||a.native)row(a,(a.native?"native":qShort(a.issuer))+" \\u00b7 "+qNum(a.bal)); });'
+'if(side==="buy"){'
+'var sh=document.createElement("div");sh.className="h";sh.textContent="Search any asset";m.appendChild(sh);'
+'var inp=document.createElement("input");inp.type="text";inp.placeholder="Code or issuer address";m.appendChild(inp);'
+'var res=document.createElement("div");m.appendChild(res);'
+'var tmr=null;'
...
+'.filter(function(a){return a.code&&/^G[A-Z2-7]{55}$/.test(a.issuer);})'
+'.sort(function(a,b){return b.tl-a.tl;})'                 // the widely-held one first
+'.forEach(function(a){var b=document.createElement("button");b.type="button";'
+'b.innerHTML="<span>"+a.code+"</span><span class=\\"s\\">"+(a.dom||qShort(a.issuer))+" \\u00b7 "+qNum(a.tl)+" trustlines</span>";'
+'b.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();qB=a;qClose();qSync();},true);'
+'res.appendChild(b);});'
+'if(!res.children.length)res.innerHTML="<div class=\\"h\\">No matches</div>";'
+'}).catch(function(){res.innerHTML="<div class=\\"h\\">Search unavailable</div>";});},260);});'
+'}'
...
+'ps.innerHTML=qS?("<span>"+qS.code+"</span><span class=\\"g\\">"+(qS.native?"native":qShort(qS.issuer))+"</span>")'
+':"<span class=\\"cv\\">Select asset</span>";'
+'pb.innerHTML=qB?("<span>"+qB.code+"</span><span class=\\"g\\">"+(qB.native?"native":qShort(qB.issuer))+"</span>")'
+':"<span class=\\"cv\\">Select asset</span>";'
+'var u=qEl(".lxq-unit");if(u)u.textContent=(qS&&qB)?(qB.code+" per "+qS.code):"";'
+'var bl=qEl(".lxq-balv");if(bl)bl.textContent=qS?(qNum(qBal(qS))+" "+qS.code):"\\u2014";'
+'var amt=parseFloat(((qEl(".lxq-amt")||{}).value||"").replace(/,/g,""))||0;'
+'var pr=parseFloat(((qEl(".lxq-price")||{}).value||"").replace(/,/g,""))||0;'
+'var tot=qEl(".lxq-totv");if(tot)tot.textContent=(qS&&qB&&amt>0&&pr>0)?(qNum(amt*pr)+" "+qB.code):"\\u2014";'
// the trustline line is a cost disclosure, not a warning to be skimmed
+'var nt=qEl(".lxq-note");'
+'if(nt){ if(qB&&!qHas(qB))nt.textContent="Buying "+qB.code+" needs a trustline. It is added in the same transaction and locks 0.5 XLM as a reserve for as long as you hold it.";'
+'else nt.textContent=""; }'
...
+'else if(amt>0&&qS&&amt>qBal(qS)+1e-9)msg="You only hold "+qNum(qBal(qS))+" "+qS.code+".";'
...
+'+\'<div class="lxq-bal"><span>Balance</span><span><span class="lxq-balv">\\u2014</span> <button type="button" class="lxq-max">MAX</button></span></div>\''
+'+\'<div class="lxq-row"><label>You buy</label><div class="lxq-pick" data-side="buy"></div></div>\''
+'+\'<div class="lxq-row"><label>Price</label><input class="lxq-price" type="text" inputmode="decimal" placeholder="0.00">\''
+'+\'<span class="lxq-unit"></span></div>\''
+'+\'<div class="lxq-tot"><span>Total you receive</span><b class="lxq-totv">\\u2014</b></div>\''
...
+'if(p){e.preventDefault();e.stopImmediatePropagation();qPick(p.getAttribute("data-side"),p.closest(".lxq-row"));return;}'
...
+'var i=qEl(".lxq-amt");if(i)i.value="";var p2=qEl(".lxq-price");if(p2)p2.value="";qSync();'
...
+'}'
+'window.__lxQOlimit=function(){qBuildUi();if(!window.__lxHoldings&&window.__lxDashBoot)try{window.__lxDashBoot();}catch(_){}qSync();};'
...
+'var addr=qAddr();'
+'if(!addr){list.innerHTML=\'<div class="lxo-empty">Connect a wallet to see your open orders.</div>\';return;}'
+'fetch(QH+"/accounts/"+addr+"/offers?limit=50&order=desc").then(function(r){return r.json();}).then(function(d){'
+'var recs=(d&&d._embedded&&d._embedded.records)||[];'
+'if(!recs.length){list.innerHTML=\'<div class="lxo-empty">No open orders.<br>A limit order you place will appear here until it fills or you cancel it.</div>\';return;}'
+'list.innerHTML="";'
...
+'row.innerHTML=\'<div class="m"><div class="p">Sell \'+qNum(amt)+" "+s.code+\' \\u2192 \'+b.code+\'</div>\''
+'+\'<div class="s">\'+qNum(pr)+" "+b.code+" per "+s.code+" \\u00b7 total "+qNum(amt*pr)+" "+b.code+\'</div></div>\''
+'+\'<button type="button" data-off="\'+o.id+\'">Cancel</button>\';'
// the row carries everything the cancel needs, so it never has to be looked up again
+'var btn=row.querySelector("[data-off]");'
...
+'set("min",fmt(minR)+" "+tSym);panel.style.display="";}else{panel.style.display="none";}}'
...
+'function fill(arr){list.innerHTML="";if(!arr.length){list.innerHTML=\'<div class="lx-am-empty">No matches</div>\';return;}arr.forEach(function(a){var b=menuItem(a);b.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();menu.remove();pick(which,a);},true);list.appendChild(b);});}fill(hs);'
