...
+'.lxq-max{background:var(--accent-soft,rgba(234,106,44,.14));color:var(--accent,#ea6a2c);border:1px solid var(--accent-soft,rgba(234,106,44,.22));border-radius:7px;font:inherit;font-size:11px;font-weight:800;padding:3px 8px;cursor:pointer;letter-spacing:.02em}'
...
+'.lxq-vt{display:inline-flex;align-items:center;justify-content:center;width:13px;height:13px;border-radius:50%;background:#35c07f;color:#fff;font-size:9px;font-weight:900;margin-left:5px;flex:0 0 auto}'
// --- Orders tab: the same field-block material as a swap row ---
+'.lxo-list{display:flex;flex-direction:column;min-height:70px}'
+'.lxo-o{display:flex;align-items:center;gap:12px;background:var(--bg);border:1px solid var(--border);border-radius:14px;padding:13px 14px;margin-bottom:10px}'
+'.lxo-o .m{flex:1 1 auto;min-width:0}'
+'.lxo-o .p{font-size:15px;font-weight:700;color:var(--text);display:flex;align-items:center;gap:7px;flex-wrap:wrap}'
+'.lxo-o .p .ar{color:var(--text-muted);font-weight:600}'
+'.lxo-o .s{font-size:12.5px;color:var(--text-soft);margin-top:5px;font-family:\'JetBrains Mono\',monospace}'
+'.lxo-o button{flex:0 0 auto;height:32px;padding:0 13px;border-radius:9px;border:1px solid var(--border);background:transparent;color:var(--text-soft);font:inherit;font-size:13px;font-weight:700;cursor:pointer}'
...
+'var QCUR=['
+'{code:"USDC",issuer:"GA5ZSEJYB37JRC5AVCIA5MOP4RHTM335X2KGX3IHOJAPP5RE34K4KZVN",dom:"centre.io",v:1},'
+'{code:"EURC",issuer:"GDHU6WRG4IEQXM5NZ4BMPKOXHW76MZM4Y2IEMFDVXBSDP6SJY4ITNPP2",dom:"circle.com",v:1},'
+'{code:"SHX",issuer:"GDSTRSHXHGJ7ZIVRBXEYE5Q74XUVCUSEKEBR7UCHEUUEK72N7I7KJ6JH",dom:"stronghold.co",v:1},'
+'{code:"AQUA",issuer:"GBNZILSTVQZ4R7IKQDGHYGY2QXL5QOFJYQMXPKWRRM5PAV7Y4M67AQUA",dom:"aqua.network",v:1},'
+'{code:"LUMOS",issuer:"GB5T2EQC2VDG2XEYQ5C2CQJ2SCB5RFPPWALUU2GQ3R5HUEGOZST55B6S",dom:"lumoscore.com",v:1}'
+'];'
...
+'var QOWN=null;'
+'function qOwn(){if(QOWN)return Promise.resolve(QOWN);'
+'return fetch("/assets/tokens/launchpad-icons.json").then(function(r){return r.json();}).then(function(m){'
+'QOWN=Object.keys(m||{}).map(function(k){var d=k.indexOf("-");if(d<1)return null;'
+'var v=m[k]||{};return {code:k.slice(0,d),issuer:k.slice(d+1),dom:"lumoscore.com",name:v.name||"",v:1,own:1};})'
+'.filter(Boolean);return QOWN;}).catch(function(){QOWN=[];return QOWN;});}'
...
+'if(a.v){var vt=document.createElement("span");vt.className="lxq-vt";vt.textContent="\u2713";cd.appendChild(vt);}'
...
+'head("Your assets");'
+'var hs=qHold();if(!hs.length)head("No balances yet");'
+'hs.forEach(function(a){ if(a.bal>0||a.native)add(a,qNum(a.bal)); });'
+'if(side==="buy"){'
+'head("Verified");QCUR.forEach(function(a){ if(!qHas(a))add(a); });'
+'var oh=head("On LumosCore");var ob=document.createElement("div");m.appendChild(ob);'
+'qOwn().then(function(list){var n=0;list.forEach(function(a){'
+'if(qSame(a,qS)||qHas(a))return;n++;var b=qRow(a,"");'
+'b.addEventListener("click",function(e){e.preventDefault();e.stopImmediatePropagation();qB=a;qClose();qSync();},true);'
+'ob.appendChild(b);});if(!n)oh.remove();});'
...
+'if(a.v){var vt=document.createElement("span");vt.className="lxq-vt";vt.textContent="✓";el.appendChild(vt);}}'
...
+'+\'<span><span class="bal">Balance: <span class="lxq-balv">\\u2014</span></span> <button type="button" class="lxq-max">MAX</button></span></div>\''
+'+\'<div class="lxq-lin"><div class="lxq-pick" data-side="sell"></div>\''
+'+\'<input class="lxq-in lxq-amt" type="text" inputmode="decimal" placeholder="0.00"></div></div>\''
+'+\'<div class="lxq-fld"><div class="lxq-lab"><span>You buy</span></div>\''
+'+\'<div class="lxq-lin"><div class="lxq-pick" data-side="buy"></div></div></div>\''
+'+\'<div class="lxq-fld"><div class="lxq-lab"><span>Price</span><span class="lxq-unit"></span></div>\''
+'+\'<div class="lxq-lin"><input class="lxq-in lxq-price" type="text" inputmode="decimal" placeholder="0.00"></div></div>\''
+'+\'<div class="lxq-meta"><span>Total you receive</span><b class="lxq-totv">\\u2014</b></div>\''
