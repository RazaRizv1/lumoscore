...
const SCRIPT = '<script id="lx-dashqa">(function(){'
  ...
  // A scroll that begins on a card must not count as a tap -- see the note in the transform.
  + 'var _qsx=0,_qsy=0,_qmoved=false;'
  + 'document.addEventListener("touchstart",function(e){'
  + 'if(!(e.target&&e.target.closest&&e.target.closest(".quick-card")))return;'
  + 'var t=e.touches&&e.touches[0]; if(!t)return;'
  + '_qsx=t.clientX;_qsy=t.clientY;_qmoved=false;'
  + '},{passive:true,capture:true});'
  + 'document.addEventListener("touchmove",function(e){'
  + 'var t=e.touches&&e.touches[0]; if(!t)return;'
  + 'if(Math.abs(t.clientX-_qsx)>10||Math.abs(t.clientY-_qsy)>10)_qmoved=true;'
  + '},{passive:true,capture:true});'
