...
+'/* Seed window.__lxLogos from our published registry. Every icon path on this page -- lpIco, ilogo,'
+' actBg, selectSendAsset, the harvesters -- already reads that cache, so filling it is the whole fix and'
+' no call site changes. Keyed by CODE like the cache itself: that is the existing design, and the'
+' harvesters overwrite per-issuer results on top anyway. */'
+'(function(){try{var R=window.__lxTokenRegistry;if(R){var L0=(window.__lxLogos=window.__lxLogos||{});for(var k0 in R){if(!Object.prototype.hasOwnProperty.call(R,k0))continue;var d0=k0.indexOf("-");if(d0<1)continue;var c0=k0.slice(0,d0),u0=R[k0]&&R[k0].image;if(typeof u0==="string"&&u0.charAt(0)==="/"&&u0.indexOf("//")!==0&&!L0[c0])L0[c0]=u0;}}}catch(_){}try{fetch("/assets/tokens/launchpad-icons.json").then(function(r){return r.ok?r.json():null;}).then(function(m){if(!m||typeof m!=="object"||m.constructor===Array)return;var L=(window.__lxLogos=window.__lxLogos||{});for(var k in m){if(!Object.prototype.hasOwnProperty.call(m,k))continue;var d=k.indexOf("-");if(d<1)continue;var c=k.slice(0,d),v=m[k],u=(v&&typeof v==="object")?v.image:v;if(typeof u!=="string"||u.charAt(0)!=="/"||u.indexOf("//")===0)continue;if(!L[c])L[c]=u;}try{lxHealAllLogos&&lxHealAllLogos(document);}catch(_){}try{lxFixLpIcons&&lxFixLpIcons();}catch(_){}}).catch(function(){});}catch(_){}})();'
