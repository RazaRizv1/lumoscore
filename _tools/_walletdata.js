...
+'function wireSendAssetPicker(){if(window.__lxPickerWired)return;window.__lxPickerWired=1;document.addEventListener("click",function(e){var pick=e.target&&e.target.closest?e.target.closest(".asset-pick"):null;if(pick){var m=pick.closest(".modal-overlay");if(m&&m.querySelector("input.mono")){e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();openAssetMenu(m,pick);return;}}if(!(e.target.closest&&e.target.closest(".lx-asset-menu"))){var mm=document.querySelector(".lx-asset-menu");if(mm)mm.remove();}},true);}'
...
+'function orderCount(){var left=document.querySelectorAll(".order-row[data-oid],.lxmw-row[data-oid]").length;var oh=findH2("Open Orders");if(oh){var m=oh.querySelector(".meta");if(m)m.textContent=left+" active";}if(!left){block.innerHTML=\'<div style="padding:26px 22px;text-align:center;color:var(--text-muted);font-size:14px">No open orders on the DEX</div>\';var cah0=findCancelAll();if(cah0)cah0.style.display="none";}return left;}'
+'if(!window.__lxCancelWired){window.__lxCancelWired=1;document.addEventListener("click",function(e){var b=e.target.closest&&e.target.closest(".order-cancel");if(!b)return;e.preventDefault();e.stopPropagation();var row=b.closest(".order-row,.lxmw-row[data-oid]");if(!row||b.disabled)return;var ot=b.textContent;b.disabled=true;b.textContent="Signing\\u2026";lxCancel(row).then(function(){b.textContent="Cancelled";row.style.transition="opacity .25s,transform .25s";row.style.opacity="0";row.style.transform="translateX(-8px)";setTimeout(function(){row.remove();orderCount();},260);}).catch(function(err){b.disabled=false;b.textContent=ot;lxToast((err&&err.message)||"Cancel failed");});});}'
+'var cah=findCancelAll();if(cah&&!cah.__lxAll){cah.__lxAll=1;cah.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();var rows=[].slice.call(document.querySelectorAll(".order-row[data-oid],.lxmw-row[data-oid]"));var _seen={};rows=rows.filter(function(r){var id=r.getAttribute("data-oid");if(!id||_seen[id])return false;_seen[id]=1;return true;});if(!rows.length)return;if(cah.disabled)return;var ot=cah.textContent;cah.disabled=true;cah.textContent="Signing\\u2026";lxCancelAll(rows).then(function(){rows.forEach(function(r){if(r.parentNode)r.parentNode.removeChild(r);});orderCount();cah.disabled=false;cah.textContent=ot;}).catch(function(err){cah.disabled=false;cah.textContent=ot;lxToast((err&&err.message)||"Cancel all failed");});},true);}}'
