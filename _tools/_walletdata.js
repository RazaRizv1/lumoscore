...
+'if(ex){ if(ev&&ex.__lxEvt===ev)return; if((Date.now()-(+ex.getAttribute("data-lxopened")||0))<400)return; ex.remove(); return; }var hs=window.__lxHoldings||[];if(!hs.length)return;var menu=document.createElement("div");menu.className="lx-asset-menu lx-hassearch";'
...
+'menu.setAttribute("data-lxopened",String(Date.now()));menu.__lxEvt=ev||null;document.body.appendChild(menu);var r=pick.getBoundingClientRect();menu.style.position="fixed";menu.style.top=(r.bottom+6)+"px";menu.style.left=Math.max(8,r.right-230)+"px";menu.style.zIndex="100000";setTimeout(function(){try{si.focus();}catch(_){}},30);}'
