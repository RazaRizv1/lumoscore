...
// The scan control sits left of Paste and is square, so the two read as one pair rather than as a
// button and an afterthought.
+'.lx-scan-btn{right:62px;padding:5px 8px;display:inline-flex;align-items:center;justify-content:center}'
+'.lx-scan-btn svg{display:block}'
// The recipient placeholder read "G... or domain.com*username" and is now "G... Stellar address".
// Two reasons. It PROMISED SOMETHING THE APP REFUSES: the only destination check anywhere in send is
// /^G[A-Z2-7]{55}$/ (validateSend and again in wireSend) -- there is no federation lookup in this
// codebase, so anyone typing a federated name was rejected. It was also written backwards; SEP-2
// federation addresses are name*domain, not domain*name. And it did not fit: measured 201px of
// JetBrains Mono 16.5px against a 203.6px text area at 375px, so it clipped on every narrower phone
// (360px is the common one). The replacement is 20 characters and fits down to a 320px screen.
// It KEEPS the literal "G..." on purpose -- the enhancer finds this input by /G\.\.\.|domain/i, and a
// placeholder starting "Stellar" with a unicode ellipsis would make it invisible to its own wiring.
//
// The scan button used to be placed with its own hardcoded right:62px, which assumed an exact Paste
// width. Measured at 375px: scan ended at x=294 and Paste began at x=294 -- a 0px gap, so the two
// rendered as one welded blob, and any change to the Paste label or font would have overlapped them.
// A flex row cannot collide with itself: the buttons keep a real gap whatever they contain, and the
// input's padding is measured from this bar rather than guessed at 118px.
+'.lx-recbtns{position:absolute;right:7px;top:50%;transform:translateY(-50%);display:flex;align-items:center;gap:6px;z-index:2}'
+'.lx-recbtns .lx-paste-btn{position:static;right:auto;top:auto;transform:none}'
...
+'if(!window.__lxRmWired){window.__lxRmWired=1;document.addEventListener("click",function(e){var bt=e.target&&e.target.closest?e.target.closest(".lx-rmtrust"):null;if(!bt)return;e.preventDefault();e.stopPropagation();if(bt.__lxb)return;bt.__lxb=1;var code=bt.getAttribute("data-rmc"),iss=bt.getAttribute("data-rmi"),lbl=bt.innerHTML;bt.textContent="Removing\\u2026";lxStellar().then(function(S){return Promise.all([j(H+"/accounts/"+ME),j(H+"/liquidity_pools?account="+ME+"&limit=200").catch(function(){return null;})]).then(function(_r){var acc=_r[0],_lp=_r[1];var _sh={};((acc&&acc.balances)||[]).forEach(function(b){if(b.asset_type==="liquidity_pool_shares")_sh[b.liquidity_pool_id]=+b.balance||0;});var _bld=new S.TransactionBuilder(new S.Account(ME,acc.sequence),{fee:"1000",networkPassphrase:(LX_NET==="testnet"?S.Networks.TESTNET:S.Networks.PUBLIC)});var _want=code+":"+iss,_recs=((_lp&&_lp._embedded&&_lp._embedded.records)||[]),_freed=0;_recs.forEach(function(p){try{var rs=p.reserves||[];if(rs.length!==2)return;if(!rs.some(function(x){return x.asset===_want;}))return;if((_sh[p.id]||0)!==0)return;var as=rs.map(function(x){if(x.asset==="native")return S.Asset.native();var q=x.asset.split(":");return new S.Asset(q[0],q[1]);});_bld.addOperation(S.Operation.changeTrust({asset:new S.LiquidityPoolAsset(as[0],as[1],(S.LiquidityPoolFeeV18||30)),limit:"0"}));_freed++;}catch(_){}});_bld.addOperation(S.Operation.changeTrust({asset:new S.Asset(code,iss),limit:"0"}));var tx=_bld.setTimeout(300).build();return lxTimeout(lxSign(tx.toXDR(),S),200000,"Signing timed out \\u2014 open your wallet and try again").then(function(signed){if(!signed)throw new Error("Signing cancelled");return fetch(H+"/transactions",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:"tx="+encodeURIComponent(signed)}).then(function(r){return r.json();});});});}).then(function(res){if(res&&(res.successful||res.hash)){try{lxToast(code+" trustline removed \\u2713");}catch(_){}setTimeout(function(){location.reload();},1200);}else{var x=res&&res.extras&&res.extras.result_codes;throw new Error(x?(window.lxTxMsg?window.lxTxMsg(x,"Removing the trustline"):JSON.stringify(x)):"failed");}})'
...
+'Promise.all([j(H+"/accounts/"+ME),j(H+"/liquidity_pools?account="+ME+"&limit=200").catch(function(){return null;})]).then(function(_d){var a2=_d[0],_lp2=_d[1];var bals=((a2&&a2.balances)||[]);'
+'var b=bals.filter(function(x){return x.asset_code===code&&x.asset_issuer===iss;})[0];'
+'var _sh2={};bals.forEach(function(x){if(x.asset_type==="liquidity_pool_shares")_sh2[x.liquidity_pool_id]=+x.balance||0;});'+'var _w=code+":"+iss;'+'var _mine=((_lp2&&_lp2._embedded&&_lp2._embedded.records)||[]).filter(function(p){return (p.reserves||[]).some(function(x){return x.asset===_w;});});'+'function _pn(p){return (p.reserves||[]).map(function(x){return x.asset==="native"?"XLM":x.asset.split(":")[0];}).join("/");}'+'var _held=_mine.filter(function(p){return (_sh2[p.id]||0)>0;});'
...
+'if(_held.length){say("Can\u2019t remove "+code+" yet \u2014 you have "+code+" deposited in the "+_pn(_held[0])+" pool. Withdraw it there first.");return;}'+'if(_mine.length){say("Can\u2019t remove "+code+" yet \u2014 an empty "+_pn(_mine[0])+" pool position is still holding it. Try again in a moment.");return;}'
...
// Presence, not domain. The map's VALUES are domains and an asset we vouch for without one stores "",
// so the truthiness test above answers "not verified" for exactly those. Same !==undefined rule the
// Trade list and search already use.
+'function lxVfdHas(code,iss){return !!(code&&iss)&&LX_VFD[code+"|"+iss]!==undefined;}'
...
+'var v=lxVfd(code,iss),vv=lxVfdHas(code,iss),h=\'<span class="lx-asb" data-vk="\'+esc(code+"|"+iss)+\'">\';'
+'if(vv)h+=\'<span class="lx-vfd" title="Verified issuer\'+(v?" \\u00b7 "+esc(v):"")+\'">\'+LX_CKSVG+\'</span>\';'
...
+'window.__lxVfdHas=lxVfdHas;'
// The baked map is a snapshot; the curated list changes without a build. Everything else that draws a
// tick merges the live map at runtime -- the wallet was the surface that never did, which is why an
// asset curated after the last deploy showed its domain here but no tick.
+'function lxWalMergeV(){'
...
+'var p=id.lastIndexOf("-");if(p<0)return;var k=id.slice(0,p)+"|"+id.slice(p+1);'
+'if(LX_VFD[k]===undefined){LX_VFD[k]=r.d||"";added++;}});'
+'if(added)lxWalPaintV();}).catch(function(){});}'
+'function lxWalPaintV(){try{'
+'var ns=document.querySelectorAll(".lx-asb[data-vk]");'
+'for(var i=0;i<ns.length;i++){var el=ns[i];'
+'if(LX_VFD[el.getAttribute("data-vk")]===undefined)continue;'
+'if(el.querySelector(".lx-vfd"))continue;'
+'var sp=document.createElement("span");sp.className="lx-vfd";'
+'sp.setAttribute("title","Verified issuer");sp.innerHTML=LX_CKSVG;'
...
+'function lxToast(msg,isErr){try{if(isErr){var _st=document.querySelector(".toast-stack");if(_st){var _t=document.createElement("div");_t.className="toast";var _ic=document.createElement("span");_ic.className="check-ic";_ic.style.background="#e5484d";var _ns="http://www.w3.org/2000/svg";var _sv=document.createElementNS(_ns,"svg");_sv.setAttribute("width","11");_sv.setAttribute("height","11");_sv.setAttribute("viewBox","0 0 24 24");_sv.setAttribute("fill","none");_sv.setAttribute("stroke","currentColor");_sv.setAttribute("stroke-width","3");_sv.setAttribute("stroke-linecap","round");_sv.setAttribute("stroke-linejoin","round");var _p1=document.createElementNS(_ns,"path");_p1.setAttribute("d","M18 6 L6 18");var _p2=document.createElementNS(_ns,"path");_p2.setAttribute("d","M6 6 L18 18");_sv.appendChild(_p1);_sv.appendChild(_p2);_ic.appendChild(_sv);var _sp=document.createElement("span");_sp.textContent=msg;_t.appendChild(_ic);_t.appendChild(_sp);_st.appendChild(_t);setTimeout(function(){try{_t.remove();}catch(_){}},4200);return;}}}catch(_){}try{if(!isErr&&typeof window.showToast==="function"){window.showToast(msg);return;}}catch(_){}try{var t=document.createElement("div");t.textContent=(isErr?(String.fromCharCode(10005)+"  "):"")+msg;t.style.cssText="position:fixed;left:50%;bottom:28px;transform:translateX(-50%);background:"+(isErr?"#3a1d20":"#1c1f27")+";color:#fff;border:1px solid "+(isErr?"rgba(229,72,77,.55)":"rgba(255,255,255,.16)")+";padding:10px 16px;border-radius:10px;font-size:13px;z-index:99999;box-shadow:0 10px 34px rgba(0,0,0,.45);max-width:82vw;text-align:center";document.body.appendChild(t);setTimeout(function(){t.style.transition="opacity .4s";t.style.opacity="0";setTimeout(function(){t.remove();},420);},2600);}catch(_){}}'
...
+'function lxCancelAll(rows){return lxStellar().then(function(S){return j(H+"/accounts/"+ME).then(function(acc){var account=new S.Account(ME,acc.sequence);var tb=new S.TransactionBuilder(account,{fee:String(100*Math.max(1,rows.length)),networkPassphrase:(LX_NET==="testnet"?S.Networks.TESTNET:S.Networks.PUBLIC)});rows.forEach(function(row){var sell=lxAsset(S,row.getAttribute("data-snt")==="1",row.getAttribute("data-sc"),row.getAttribute("data-si"));var buy=lxAsset(S,row.getAttribute("data-bnt")==="1",row.getAttribute("data-bc"),row.getAttribute("data-bi"));tb.addOperation(S.Operation.manageSellOffer({selling:sell,buying:buy,amount:"0",price:String(row.getAttribute("data-price")||"1"),offerId:String(row.getAttribute("data-oid"))}));});var tx=tb.setTimeout(300).build();return lxTimeout(lxSign(tx.toXDR(),S),200000,"Signing timed out \\u2014 open your wallet extension and try again").then(function(signed){if(!signed)throw new Error("Signing cancelled");return fetch(H+"/transactions",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:"tx="+encodeURIComponent(signed)}).then(function(r){return r.json();}).then(function(res){if(res&&(res.successful||res.hash))return res;var x=res&&res.extras&&res.extras.result_codes;throw new Error(x?(window.lxTxMsg?window.lxTxMsg(x,"The order"):("DEX: "+JSON.stringify(x))):"Submit failed");});});});});}'
...
+'function lxOwnLogo(code,iss){try{return fetch("/lxapi/assetlogo?v=2&asset="+encodeURIComponent(code+"-"+iss)).then(function(r){return r.ok?r.json():null;}).then(function(d){return (d&&d.image)||"";}).catch(function(){return "";});}catch(_){return Promise.resolve("");}}'
+'function lxApplyLogo(code,iss,img){if(!img)return;(window.__lxLogos=window.__lxLogos||{})[code]=img;var _a=document.querySelectorAll("[data-lxc][data-lxi]");for(var _i=0;_i<_a.length;_i++){var _x=_a[_i];if(_x.getAttribute("data-lxc")===code&&_x.getAttribute("data-lxi")===iss)lxPaintIco(_x,img);}}'
+'function lxHealAllLogos(root){root=root||document;try{var els=[].slice.call(root.querySelectorAll("[data-lxc][data-lxi]"));var need={};els.forEach(function(el){var code=el.getAttribute("data-lxc"),iss=el.getAttribute("data-lxi");if(!code||code==="XLM")return;var cached=(window.__lxLogos||{})[code];if(cached){lxPaintIco(el,cached);return;}if(!iss)return;(need[code+"|"+iss]=need[code+"|"+iss]||[]).push(el);});Object.keys(need).forEach(function(key){var parts=key.split("|"),code=parts[0],iss=parts[1];j("/lxapi/assetsearch?search="+encodeURIComponent(code)+"&limit=20").then(function(d){var recs=(d._embedded&&d._embedded.records)||[];var m=recs.filter(function(rc){return (rc.asset||"").indexOf(code+"-"+iss)===0;})[0];var ti=(m&&(m.tomlInfo||m.toml_info))||{};var img=ti.image||ti.orgLogo||"";if(!img){lxOwnLogo(code,iss).then(function(u){lxApplyLogo(code,iss,u);});return;}(window.__lxLogos=window.__lxLogos||{})[code]=img;[].slice.call(document.querySelectorAll("[data-lxc=\\x27"+code+"\\x27][data-lxi=\\x27"+iss+"\\x27]")).forEach(function(x){lxPaintIco(x,img);});}).catch(function(){});});}catch(_){}}'
...
+'fetch("/lxapi/assetlogo?v=2&asset="+encodeURIComponent(h.code+"-"+h.iss)).then(function(r){return r.ok?r.json():null;})'
...
+'function clearSend(m){if(!m)return;var a=m.querySelector(\'input[placeholder="0.00"]\');if(a)a.value="";var r=m.querySelector("input.mono");if(r)r.value="";var mo=m.querySelector(".lx-memo");if(mo)mo.value="";var rb=m.querySelector("[data-lxsendgo]")||[].slice.call(m.querySelectorAll("button")).filter(function(b){return /Review.*Send/i.test((b.textContent||"").replace(/\\s+/g," "));})[0];if(rb){rb.disabled=false;rb.classList.remove("lx-btn-off");if(rb.__lxorigHTML)rb.innerHTML=rb.__lxorigHTML;}var hs=window.__lxHoldings||[],nat=hs.filter(function(h){return h.native;})[0],sel=window.__lxSendPre||nat;if(sel)selectSendAsset(m,sel);}'
+'function wireSend(){document.addEventListener("click",function(e){var b=e.target&&e.target.closest?e.target.closest("button"):null;if(!b)return;var t=(b.textContent||"").replace(/\\s+/g," ").trim();if(!/Review.*Send/i.test(t))return;var m=b.closest(".modal-overlay");if(!m)return;var amtI=m.querySelector(\'input[placeholder="0.00"]\'),recI=m.querySelector("input.mono");if(!amtI||!recI)return;e.preventDefault();e.stopPropagation();if(b.disabled)return;if(!b.__lxorigHTML)b.__lxorigHTML=b.innerHTML;b.setAttribute("data-lxsendgo","1");var amount=parseFloat((amtI.value||"").replace(/,/g,""))||0,dest=(recI.value||"").trim(),memoI=m.querySelector(".lx-memo"),memoRaw=memoI?(memoI.value||"").trim():"",memo=memoRaw,memoT=((m.querySelector(".lx-memo-type")||{}).value||"text"),symB=m.querySelector(".asset-pick"),sym=m.__lxsym||(symB?((symB.textContent||"").match(/[A-Z0-9]{2,12}/)||["XLM"])[0]:"XLM");if(!/^G[A-Z2-7]{55}$/.test(dest)){lxToast("Enter a valid Stellar address (G\\u2026)");return;}'
...
// jsQR, self-hosted like the Stellar SDK and fetched only when someone actually taps Scan.
+'var __qrP=null;function lxQrLib(){if(__qrP)return __qrP;'
+'__qrP=new Promise(function(res,rej){if(window.jsQR)return res(window.jsQR);'
...
+'s.onerror=function(){__qrP=null;rej(new Error("Scanner failed to load"));};'
+'document.head.appendChild(s);});return __qrP;}'
// A wallet QR is either a bare address or a SEP-7 pay URI. Take the address out of whichever it is.
// Scanning is a phone action: on a desktop the camera is pointing at the user, and Paste is the
// natural gesture. Same UA test the site already uses to decide which page to serve.
+'function lxIsMob(){try{return /Android|iPhone|iPad|iPod/i.test(navigator.userAgent||"");}catch(_){return false;}}'
+'function lxQrAddr(t){t=String(t||"").trim();'
+'if(/^G[A-Z2-7]{55}$/.test(t))return t;'
+'var i=t.indexOf("destination=");'
+'if(i>=0){var v=t.slice(i+12).split("&")[0];try{v=decodeURIComponent(v);}catch(_){}'
+'if(/^G[A-Z2-7]{55}$/.test(v))return v;}'
+'var m=t.toUpperCase().match(/G[A-Z2-7]{55}/);return m?m[0]:"";}'
// The sheet. Everything that can end it goes through stop(), so the camera cannot be left running.
+'function lxQrScan(onFound){'
+'if(!window.isSecureContext){lxToast("The camera needs a secure (https) connection.",true);return;}'
+'if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia){lxToast("This browser cannot open the camera.",true);return;}'
// One scanner at a time: on touch the same finger delivers two clicks (_mobnav re-dispatches the one it suppressed),
// which opened two cameras. The losing request reports NotAllowedError, read out as a refusal nobody made.
+'if(window.__lxQrOpen)return;window.__lxQrOpen=1;'
+'var ov=document.createElement("div");'
+'ov.style.cssText="position:fixed;inset:0;z-index:2147483000;background:#000;display:flex;flex-direction:column;align-items:center;justify-content:center";'
+'var vid=document.createElement("video");vid.setAttribute("playsinline","");vid.muted=true;vid.autoplay=true;'
+'vid.style.cssText="width:100%;height:100%;object-fit:cover;position:absolute;inset:0";'
+'var frame=document.createElement("div");'
+'frame.style.cssText="position:relative;width:66vw;max-width:300px;aspect-ratio:1;border:3px solid rgba(255,255,255,.9);border-radius:18px;box-shadow:0 0 0 100vmax rgba(0,0,0,.55)";'
+'var hint=document.createElement("div");hint.textContent="Point at a Stellar address QR code";'
+'hint.style.cssText="position:absolute;left:0;right:0;bottom:14vh;text-align:center;color:#fff;font:600 14px system-ui,sans-serif;text-shadow:0 1px 3px rgba(0,0,0,.8)";'
+'var cancel=document.createElement("button");cancel.type="button";cancel.textContent="Cancel";'
+'cancel.style.cssText="position:absolute;left:50%;transform:translateX(-50%);bottom:6vh;padding:11px 26px;border-radius:999px;border:1px solid rgba(255,255,255,.35);background:rgba(0,0,0,.5);color:#fff;font:700 14px system-ui,sans-serif;cursor:pointer";'
+'ov.appendChild(vid);ov.appendChild(frame);ov.appendChild(hint);ov.appendChild(cancel);document.body.appendChild(ov);'
+'var stream=null,raf=0,dead=false;'
+'function stop(){if(dead)return;dead=true;window.__lxQrOpen=0;try{cancelAnimationFrame(raf);}catch(_){}'
...
+'cancel.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();stop();});'
...
+'var r=null;try{r=dec(d.data,w,h,{inversionAttempts:"attemptBoth"});}catch(_){}'
+'if(r&&r.data){var a=lxQrAddr(r.data);'
+'if(a){stop();onFound(a);return;}'
+'stop();lxToast("That QR code has no Stellar address in it.",true);return;}}'
+'raf=requestAnimationFrame(tick);})();});});})'
+'.catch(function(err){stop();'
+'var n=(err&&err.name)||"";'
+'if(n==="NotAllowedError"||n==="SecurityError")lxToast("Camera permission was declined.",true);'
+'else if(n==="NotFoundError"||n==="OverconstrainedError")lxToast("No camera found on this device.",true);'
+'else lxToast(((err&&err.message)||"Could not open the camera."),true);});}'
...
+'var rec=m.querySelector("input.mono")||[].slice.call(m.querySelectorAll("input")).filter(function(i){return /G\\.\\.\\.|domain/i.test(i.placeholder||"");})[0];if(rec&&!rec.__lxPaste){rec.__lxPaste=1;rec.placeholder="G... Stellar address";var wrap=document.createElement("div");wrap.style.position="relative";rec.parentNode.insertBefore(wrap,rec);wrap.appendChild(rec);var _qmob=lxIsMob();rec.style.paddingRight=_qmob?"118px":"70px";var pb=document.createElement("button");pb.type="button";pb.className="lx-paste-btn";pb.textContent="Paste";function _pbRun(){/* EXACTLY ONE READ PER TAP (RAZA 2026-09-21: paste "back on Tablet and mobile"). Two reads started per tap -- the copy of the click that _mobnav dispatches inside the touchend, and the touchend listener -- and a second read arriving while the own paste prompt of the browser is open cancels or rejects it. Nothing may re-ask while a read is pending. */var _now=Date.now(),G=pb.__lxG;if(!G||_now-G.t>900){G=pb.__lxG={ok:false,busy:false,timer:0};}G.t=_now;if(G.ok||G.busy)return;G.busy=true;function _win(x){G.ok=true;G.busy=false;if(G.timer){clearTimeout(G.timer);G.timer=0;}rec.value=x;rec.dispatchEvent(new Event("input",{bubbles:true}));rec.focus();}function _fail(){G.busy=false;if(G.ok)return;if(G.timer)clearTimeout(G.timer);G.timer=setTimeout(function(){G.timer=0;if(!G.ok)_man();},450);}function _man(){try{rec.focus();}catch(_){}var touch=false;try{touch=window.matchMedia("(pointer:coarse)").matches;}catch(_){}lxToast(touch?"Use Paste on the keyboard, or long-press the field and choose Paste":"Couldn\u2019t read the clipboard \u2014 press Ctrl+V to paste",true);}try{window.focus();}catch(_){}var _f=true;try{_f=document.hasFocus();}catch(_){}/* Always attempt the read: Android reports clipboard-read as "denied" by default and shows its own paste confirmation only when a page asks, so gating on that query suppressed the call that works. */ if(!(navigator.clipboard&&navigator.clipboard.readText)){_fail();return;}navigator.clipboard.readText().then(function(x){x=String(x==null?"":x).trim();if(!x){_fail();return;}_win(x);}).catch(_fail);}var _pts=null;pb.addEventListener("touchstart",function(e){var q=e.touches&&e.touches[0];_pts=q?{x:q.clientX,y:q.clientY,t:Date.now()}:null;},{passive:true});pb.addEventListener("touchend",function(e){var q=e.changedTouches&&e.changedTouches[0];if(_pts&&q&&(Math.abs(q.clientX-_pts.x)>=12||Math.abs(q.clientY-_pts.y)>=12||(Date.now()-_pts.t)>=600))return;pb.__lxTouchT=Date.now();_pbRun();},{passive:true});pb.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();if(e.__lxBridged)return;if(pb.__lxTouchT&&Date.now()-pb.__lxTouchT<900)return;_pbRun();},true);var bar=document.createElement("div");bar.className="lx-recbtns";wrap.appendChild(bar);bar.appendChild(pb);if(_qmob){var sb=document.createElement("button");sb.type="button";sb.className="lx-paste-btn lx-scan-btn";sb.setAttribute("aria-label","Scan a QR code");sb.title="Scan a QR code";sb.innerHTML=\'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 8V5a2 2 0 0 1 2-2h3"></path><path d="M16 3h3a2 2 0 0 1 2 2v3"></path><path d="M21 16v3a2 2 0 0 1-2 2h-3"></path><path d="M8 21H5a2 2 0 0 1-2-2v-3"></path><path d="M7 12h10"></path></svg>\';sb.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();lxQrScan(function(addr){rec.value=addr;rec.dispatchEvent(new Event("input",{bubbles:true}));try{rec.focus();}catch(_){}lxToast("Address scanned");});});bar.insertBefore(sb,pb);}function _fitPad(){try{var _bw=bar.getBoundingClientRect().width;if(_bw>0)rec.style.paddingRight=Math.round(_bw+14)+"px";}catch(_){}}_fitPad();setTimeout(_fitPad,60);setTimeout(_fitPad,400);window.addEventListener("resize",_fitPad);}var fp=m.querySelector(".asset-pick");if(fp&&!m.querySelector(".lx-asset-pick")){fp.style.display="none";var ap=document.createElement("button");ap.type="button";ap.className="lx-asset-pick";ap.setAttribute("data-lx-noswap","");ap.innerHTML=\'<span class="lx-ap-ico" data-lx-noswap=""></span><span class="lx-ap-code" data-lx-noswap="">XLM</span><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>\';fp.parentNode.insertBefore(ap,fp.nextSibling);ap.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();openAssetMenu(m,ap,e);});m.__lxsym="XLM";var nat=(window.__lxHoldings||[]).filter(function(h){return h.native;})[0],sel=window.__lxSendPre||nat;if(sel)selectSendAsset(m,sel);else{var _sn=0,_siv=setInterval(function(){var _s2=window.__lxSendPre||(window.__lxHoldings||[]).filter(function(h){return h.native;})[0];if(_s2){clearInterval(_siv);try{selectSendAsset(m,_s2);}catch(_){}}else if(++_sn>60)clearInterval(_siv);},200);}}var famt=m.querySelector(".field-amt"),meta=m.querySelector(".field-meta"),maxb=meta?meta.querySelector(".pill-btn"):null;if(famt&&maxb&&!maxb.__lxmoved){maxb.__lxmoved=1;famt.classList.add("lx-send-amt");maxb.classList.add("lx-max-inline");famt.appendChild(maxb);}}catch(_){}}'
...
// Send reset. clearSend used to run synchronously on the click, against document.querySelector(
// "input.mono") -- the first such input anywhere in the page, whose .modal-overlay ancestor is not
// necessarily the dialog about to open, and which is read before that dialog exists at all. So the
// second Send came up still carrying the previous amount, address and memo. It now resets the modal
// that actually opened, once per open rather than on both passes: run() fires at 50ms and again at
// 180ms, and clearing twice would wipe anything typed in between.
+'function wireBalMax(){fixBalances(document);document.addEventListener("click",function(e){var b=e.target&&e.target.closest?e.target.closest("button"):null;if(!b)return;var t=(b.textContent||"").trim();if(t==="Send"||t.indexOf("Receive")>=0||/^swap$/i.test(t)){var cleared=false;var run=function(){var mm=lxOpenModal();fixBalances(mm||document);if(t==="Send"&&mm){if(!cleared){cleared=true;clearSend(mm);}enhanceSend(mm);validateSend(mm);}};setTimeout(run,50);setTimeout(run,180);return;}if(!/^max$/i.test(t))return;var m=b.closest(".modal-overlay");if(!m||!m.querySelector("input.mono"))return;var doMax=function(){var sym=m.__lxsym||"XLM",num;if(sym==="XLM"){num=(window.__lxMaxXLM!=null?window.__lxMaxXLM:window.__lxNative);}else{var h=(window.__lxHoldings||[]).filter(function(x){return x.code===sym;})[0];num=h?h.bal:null;}if(num==null||!isFinite(num))return;var inp=m.querySelector(\'input[placeholder="0.00"]\')||m.querySelector("input:not(.mono)");if(inp){var _f=Math.floor(num*1e7)/1e7;inp.value=_f>0?_f.toFixed(7).replace(/0+$/,"").replace(/\\.$/,""):"0";inp.dispatchEvent(new Event("input",{bubbles:true}));}};requestAnimationFrame(doMax);setTimeout(doMax,90);},false);}'
...
// THE LOADING SHAPE WAS BUILT AND THEN HIDDEN (RAZA 2026-09-17: "On Stellar it doesnt have this loading on the
// portfolio chart in wallet like there is on XRPL. Also add this on Stellar too"). The skeleton markup, and every class
// styling it, have been here all along -- what the line below used to do was paint it and REMOVE lx-chart-ready in the
// same breath, and the design gates the box on exactly that class: #heroChart:not(.lx-chart-ready){visibility:hidden}.
// So the box stayed blank until the real chart landed. The XRPL fork ADDS the class with the skeleton, which is why it
// has the animation this one was missing; it now does the same. Nothing new is styled and nothing new is fetched.
...
// Published so a flow that changes the account ON CHAIN can put the page back in step without the
