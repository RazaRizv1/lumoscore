...
+ 'fetch("https://api.stellar.expert/explorer/public/asset/"+job.code+"-"+job.iss)'
+ '.then(function(r){if(!r.ok)throw 0;return r.json();}).then(function(d){'
+ 'var t=(d&&(d.tomlInfo||d.toml_info))||{};LOGOFAILS=0;storeLogo(job.code,job.iss,t.image||"");},'
...
+ 'var pair="Pool "+String(b.liquidity_pool_id).slice(0,6)+"\\u2026",sub="loading\\u2026",amt="";'
...
+ 'html+=\'<div class="lxmw-row"><div class="lxmw-ico"></div><div><div class="lxmw-nm">\'+esc(pair)+\'</div>\''
+ '+\'<div class="lxmw-sub">\'+esc(sub)+\'</div></div>\''
+ '+\'<div class="lxmw-amt"><div class="a">\'+esc(fmt(shares))+\'</div><div class="u">shares \'+esc(amt)+\'</div></div></div>\';});'
+ 'list.innerHTML=html;}'
