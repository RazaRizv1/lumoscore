...
+'+(r.who?(\'<a class="meta lx-actwho" href="/account/stellar/\'+esc(r.who)+\'" title="\'+esc(r.who)+\'">\'+esc(shortAddr(r.who))+\'</a>\'):\'<div class="meta"></div>\')'
+'+\'</div>\''
...
+'rows[i].ic=de.ic;rows[i].cls=de.cls;rows[i].type=de.type;'
+'list.innerHTML=rows.map(feedRow).join("");'
+'}).catch(function(){});});'
+'}).catch(function(){});}'
