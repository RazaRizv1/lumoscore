...
const PAGE_TITLE = LZ_LIVE
  ? 'Bridge USDC and USDT0 across 36 chains | LumosCore'
...
// the bridge, with its mark
+'.lx-buse-r{display:inline-flex;align-items:center;gap:7px;font-weight:600;color:var(--text,#0e0e10);white-space:nowrap}'
+'.lx-buse-r img{width:18px;height:18px;border-radius:50%;object-fit:cover;flex:0 0 18px;display:block}'
+'.brm-recv .lx-buse-r{font-size:inherit;vertical-align:-4px}.brm-recv .lx-buse-r img{width:15px;height:15px;flex-basis:15px}'
// asset logo + its network, one mark per end of the transfer. NOT .br-ic: that class clips its overflow, which is
// exactly where the network badge sits.
+'.lx-tic{position:relative;width:26px;height:26px;flex:0 0 26px;display:block}'
+'.lx-tic>img{width:100%;height:100%;border-radius:50%;object-fit:cover;display:block;background:var(--surface,#fff)}'
+'.lx-tl{display:block;width:100%;height:100%;border-radius:50%;background:rgba(127,127,140,.2)}'
+'.lx-tl::before{content:attr(data-l);display:flex;align-items:center;justify-content:center;width:100%;height:100%;'
+'font:700 12px/1 "Hanken Grotesk",system-ui,sans-serif;color:var(--text,#0e0e10)}'
+'.lx-tnet{position:absolute;right:-4px;bottom:-4px;width:14px;height:14px;border-radius:50%;overflow:hidden;'
+'background:var(--surface,#fff);box-shadow:0 0 0 1.6px var(--surface,#fff);display:block}'
+'.lx-tnet img,.lx-tnet svg,.lx-tnet>span{width:100%!important;height:100%!important;object-fit:cover;display:block;border-radius:50%}'
// the Stellar mark is an inline svg, and it carries its own dark disc
+'.lx-tnet-x{background:#000;box-shadow:0 0 0 1.6px var(--surface,#fff)}'
