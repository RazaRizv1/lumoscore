...
function lxCctpBridgeFull(destDomain, sourceAmountHuman, recipient, sourceSpec, onStatus, netUsdcTarget){
  ...
  return lxCctpSdk().then(function(S){ return lxCctpSigner().then(function(f){
    var pk;
    var deferredFee=null;   // AUDIT #2 (FUNDS): USDC-path fee runs AFTER a successful burn, never before
    ...
    return Promise.resolve(f.requestAccess?f.requestAccess():null).then(function(){ return f.getAddress?f.getAddress():f.getPublicKey(); }).then(function(a){
      ...
      if(isUSDC){
        ...
        deferredFee=function(){
          onStatus("Collecting fee ("+(feeRate*100).toFixed(1)+"% → XLM)…");
          return lxStrictPath(C,{code:"USDC",issuer:UI},feeAmt.toFixed(7),{native:true}).then(function(p){ return acc().then(function(ad){
            var tb=new S.TransactionBuilder(new S.Account(pk,ad.sequence),{fee:"1000",networkPassphrase:C.passphrase})
              .addOperation(S.Operation.pathPaymentStrictSend({sendAsset:new S.Asset("USDC",UI),sendAmount:feeAmt.toFixed(7),destination:C.feeCollector,destAsset:S.Asset.native(),destMin:(p.out*0.97).toFixed(7),path:lxToAssets(S,p.path)}))
      ...
      return lxStrictPath(C,sourceSpec,swapAmt.toFixed(7),{code:"USDC",issuer:UI}).then(function(pSwap){
        ...
        return feePathP.then(function(pFee){ return usdcBal().then(function(before){ return acc().then(function(ad){
          ...
          var tb=_tbb.addOperation(S.Operation.pathPaymentStrictSend({sendAsset:src,sendAmount:swapAmt.toFixed(7),destination:pk,destAsset:new S.Asset("USDC",UI),destMin:(pSwap.out*0.97).toFixed(7),path:lxToAssets(S,pSwap.path)}))
            .addOperation(feeOp)
            .setTimeout(120).build();
