...
window.lxLaunchToken=function(draft, onStatus, opts){
  ...
  return lxLpSdk().then(function(S){
    ...
    return start.then(function(pk){
      ...
      onStatus("Token launched ✓"); try{ lxLpSaveMeta(result); }catch(_){} return result;
    });
  });
};
...
function lxLpSaveMeta(result){
  try{
    if(!result || !result.code || !result.issuer || !result.mintHash) return;
    ...
    fetch("/lxapi/mintmeta",{
      method:"POST", headers:{"content-type":"application/json"},
      body:JSON.stringify({ code:result.code, issuer:result.issuer, txHash:result.mintHash,
        name:d.name||"", description:d.desc||"", website:d.website||"",
        twitter:d.twitter||"", telegram:d.telegram||"", logo:logo })
    }).catch(function(){});
  }catch(_){}
}
