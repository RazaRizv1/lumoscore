...
const SCRIPT = `<script id="lx-dxadata">(function(){
  ...
  var LXMASK=".crumb span:last-child,.price-display .big,.price-display .change-pill,.price-display .meta b.mono,.ohlc-strip .pair .v,.dxa-perf-grid .dxa-perf-cell .ch,.tabs-bar .tab .count,.dxa-trade-frow .mono,.dxa-trade-summary .dxa-tsum-row .mono,.dxa-hl-stat .val";
  ...
  function usd(x){x=+x||0;if(x>=1)return "$"+x.toLocaleString("en-US",{maximumFractionDigits:2});if(x>=0.01)return "$"+x.toFixed(4);if(x>0)return "$"+x.toFixed(8);return "$0";}
  function xlmAmt(x){x=+x||0;if(x>=1000)return Math.round(x).toLocaleString("en-US");if(x>=1)return (+x.toFixed(4)).toString();if(x>0)return (+x.toPrecision(4)).toString();return "0";}
  ...
  function applyPerf(){
    var g=q(".dxa-perf-grid"); if(!g)return;
    var TFS=["1h","24h","7d","1m","3m","6m"];
    qa(".dxa-perf-grid .dxa-perf-cell").forEach(function(cell,i){
      ...
      if(v!=null){ setText(ch,(v>=0?"+":"")+v.toFixed(2)+"%"); var cls="dxa-perf-cell "+(v>=0?"up":"down"); if(cell.className!==cls)cell.className=cls; }
      else { setText(ch,"—"); if(cell.className!=="dxa-perf-cell lx-nd")cell.className="dxa-perf-cell lx-nd"; }
    });
  }
  ...
  function chartFromTrades(tf){
    ...
    if(use.length<2){ chartEmpty(src.length?1:0); return; }
  ...
  function setRecvUsd(usdv){ var rf=recvEl(); if(!rf)return; var lc=rf.querySelector(".dxa-trade-frow .lc-money"); if(lc){ var t=usdv>0?("$"+(usdv>=1?usdv.toLocaleString("en-US",{maximumFractionDigits:2}):usdv.toFixed(4))):"$0.00"; if(lc.getAttribute("data-orig")!=null)lc.setAttribute("data-orig",t); if(lc.textContent!==t)lc.textContent=t; } }
  ...
  function reAssertView(){ var rin=recvInput(), ra=recvAsset();
    if(_dxView){ if(rin&&rin.value!==_dxView.recv)rin.value=_dxView.recv; setRecvUsd(_dxView.usd);
    ...
    else { if(rin&&rin.value!=="")rin.value=""; setRecvUsd(0); setSummary("Rate","\\u2014"); setSummary("Price impact","\\u2014"); setSummary("Min received","\\u2014"); dxSmartBadge(null); }
  }
  ...
  function dxQuote(){
    ...
    if(spot>0){ var est=net*spot; _dxView={recv:xlmAmt(est), usd:est*(ra.native?xlmUsd:priceUsd()), rate:"1 "+pcode+" = "+(+spot.toPrecision(6))+" "+rcode, impact:"\\u2026", impUp:true, minR:xlmAmt(est*(1-SLIP/100))+" "+rcode}; reAssertView(); }
  ...
  function applySwap(){
    ...
    pane.addEventListener("input",function(e){ var t=e.target; if(t&&t.tagName==="INPUT"){ var pf=payEl(); if(pf&&pf.contains(t)){ _dxQuick=null; qa(".dxa-pane-swap .dxa-trade-quick button").forEach(function(o){o.classList.remove("lxq-active");}); dxQuote(); } } },true);
  }
