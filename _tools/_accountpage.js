...
const SCRIPT = `<script id="lx-accdata">(function(){
  ...
  function readAddr(){
    ...
    if(!a){ var parts=location.pathname.split("/"); var last=parts[parts.length-1]||parts[parts.length-2]||"";
      if(last.length===56&&(last.charAt(0)==="G"||last.charAt(0)==="C"))a=last; }
    a=(a||"").trim();
    // G = account, C = Soroban contract. Both are 56-char StrKey; only one has balances.
    return (a.length===56&&(a.charAt(0)==="G"||a.charAt(0)==="C"))?a:"";
  }
  var ADDR=readAddr();
  var IS_CONTRACT=(ADDR.charAt(0)==="C");
    ...
    tb.innerHTML=rows.map(function(a){
      ...
      return '<tr class="acc-row" data-code="'+esc(a.code)+'" data-iss="'+esc(a.issuer||"")+'">'
        +'<td><span class="acc-pair"><span class="acc-ico" data-c="'+esc(a.code)+'" data-i="'+esc(a.issuer||"")+'"></span>'
          +'<span><span class="acc-code">'+esc(a.code)+'</span>'+(a.native?"":vtick(a.code,a.issuer))
  ...
  function boot(){
    ...
    paintHeader();
    if(IS_CONTRACT){
      // Horizon answers 400 for /accounts/<C...>: a contract has no balances, trustlines or operation
      // history at that API, and the explorer endpoint that does is not reachable from a browser. Say so
      // instead of showing four dashes and letting the reader assume the wallet is empty.
      fail("This is a Soroban contract, not a wallet. Contract balances are not served by Horizon's account API "
        + String.fromCharCode(8212) + " open it on the explorer to see its state.");
      return;
    }
