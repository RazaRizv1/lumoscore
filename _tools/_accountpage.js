...
const MAIN_INNER = `
      ...
      <section class="acc-card">
        <div class="acc-card-head"><h2>Assets</h2><span class="acc-n" id="accAssetsN"></span></div>
        <div class="acc-scroll">
          <table class="acc-tbl" id="accAssetsTbl">
            <thead><tr><th>Asset</th><th class="num">Balance</th><th class="num">Price</th><th class="num">Value</th><th class="num acc-hide-sm">Share</th></tr></thead>
            <tbody><tr class="acc-empty-row"><td colspan="5"><div class="acc-empty">Loading assets&#8230;</div></td></tr></tbody>
          </table>
        </div>
      </section>

      <section class="acc-card">
        <div class="acc-card-head"><h2>Liquidity pools</h2><span class="acc-n" id="accPoolsN"></span></div>
        <div class="acc-scroll">
          <table class="acc-tbl" id="accPoolsTbl">
            ...
            <tbody><tr class="acc-empty-row"><td colspan="4"><div class="acc-empty">Loading pools&#8230;</div></td></tr></tbody>
          </table>
        </div>
      </section>
...
const SCRIPT = `<script id="lx-accdata">(function(){
  ...
  function paintLogos(){ qa(".acc-ico[data-c]").forEach(function(el){
  ...
  function tomlField(block,k){ var lines=block.split(String.fromCharCode(10));
    for(var i=0;i<lines.length;i++){ var ln=lines[i].trim(), eq=ln.indexOf("=");
      ...
      return v; }
    return ""; }
  function fetchLogo(c,i){ if(!c||c==="XLM"||!i)return; if(brand(c,i)||IMG[key(c,i)])return;
  ...
  function setMore(tblSel,kind,shown,total){
    ...
    var card=tbl.closest(".acc-card"); if(!card)return;
  ...
  function renderAssets(){
    ...
    rows.forEach(function(a){ if(!a.native&&!a.__px){ a.__px=1; loadAssetPx(a); fetchLogo(a.code,a.issuer); } });
    tb.innerHTML=rows.map(function(a){
      var share=(tot>0&&a.usd)?(a.usd/tot*100):0;
      return '<tr class="acc-row" data-code="'+esc(a.code)+'" data-iss="'+esc(a.issuer||"")+'">'
        +'<td><span class="acc-pair"><span class="acc-ico" data-c="'+esc(a.code)+'" data-i="'+esc(a.issuer||"")+'"></span>'
          +'<span><span class="acc-code">'+esc(a.code)+'</span>'
          +'<span class="acc-dom">'+esc(a.dom||shortG(a.issuer))+'</span></span></span></td>'
        +'<td class="num">'+amt(a.bal)+'</td>'
        +'<td class="num">'+(a.native?"1 XLM":(a.px>0?amt(a.px)+" XLM":DASH))+'</td>'
        +'<td class="num">'+(a.usd!=null&&xlmUsd>0?usd(a.usd):DASH)+'</td>'
        +'<td class="num acc-hide-sm"><span class="acc-bar" style="display:inline-block;width:74px" title="'+share.toFixed(1)+'%">'
          +'<i style="width:'+Math.max(2,Math.min(100,share)).toFixed(1)+'%"></i></span></td>'
        +'</tr>'; }).join("");
  ...
  function renderPools(){
    ...
    if(!POOLS.length){ tb.innerHTML='<tr class="acc-empty-row"><td colspan="4"><div class="acc-empty">'
    ...
    tb.innerHTML=plist.map(function(p){
      return '<tr class="acc-row" data-pool="'+esc(p.id)+'">'
        +'<td><span class="acc-pair"><span class="acc-icos">'
          +'<span class="acc-ico" data-c="'+esc(p.a)+'" data-i="'+esc(p.ai||"")+'"></span>'
          +'<span class="acc-ico" data-c="'+esc(p.b)+'" data-i="'+esc(p.bi||"")+'"></span></span>'
  ...
  function boot(){
    if(!ADDR){ fail("No Stellar address in this link."); return; }
    paintHeader();
    ...
    j(H+"/accounts/"+ADDR).then(function(acc){
      ...
      DONE.pools=1;
      paintHeader(); renderStats(); renderAssets(); renderPools();
