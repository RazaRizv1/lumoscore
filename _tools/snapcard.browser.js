...
(function () {
  ...
  function drawChart(g, R, series, vols, d, th, times) {
    ...
    for (i = 0; i <= TICKS; i++) {
      ...
      tx(g, axisNum(vv), R.x + R.w + 8, yy - aS / 2, aS, '500', MONO, th.axis);
    }
  ...
  function render(m, img, flame, portrait) {
    ...
    var probeS = Math.max(9, Math.round(chartH * 0.052));
    var widest = 0;
    for (var pk = 0; pk < m.series.length; pk++) {
      var pw = wid(g, axisNum(m.series[pk]), probeS, '500', MONO);
      if (pw > widest) widest = pw;
    }
    var axisGutter = Math.ceil(widest) + 14;
    var R = { x: P.l, y: chartTop, w: W - P.l - P.r - axisGutter, h: chartH, xAxisY: timeTop };
    ...
    if (wins.length) {
      ...
      var stampW = Math.ceil(wid(g, stamp(new Date()), 12, '400', MONO));
      var availW = big ? (W - P.l - P.r) : (W - P.l - P.r - qrBox - 16 - stampW - 20);
    ...
    if (big) {
      ...
      tx(g, stampS, P.l, footTop + (qrBox - 16) / 2, 16, '400', MONO, th.axis);
    } else {
      // Beside the QR, not at P.l: on this layout the footer band IS the stat-tile row, so a stamp
      // left-aligned to the card margin lands on top of the first tile.
      var fy = winsTop, fh = winH;
      var qx2 = W - P.r - qrBox, qy2 = fy + fh - qrBox;
      drawQR(g, m.url, qx2, qy2, qrBox, th);
      tx(g, stampS, qx2 - 16, qy2 + (qrBox - 12) / 2, 12, '400', MONO, th.axis, 'right');
    }
