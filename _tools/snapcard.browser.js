...
(function () {
  ...
  function drawQR(g, text, x, y, box, th) {
    ...
    for (var i = 0; i < res.size; i++) for (var j = 0; j < res.size; j++) {
      ...
      // Ceil the module box so neighbours meet: a sub-pixel gap between dark modules is exactly the
      // artefact that makes a small symbol fail to decode.
      g.fillRect(Math.round(x + padPx + j * m), Math.round(y + padPx + i * m), Math.ceil(m), Math.ceil(m));
    }
    return box;
  }
