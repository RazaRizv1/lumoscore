...
const SCRIPT = `<script id="lx-searchassets">(function(){
  ...
  function lxSeaLogos(root){ var seen=(window.__lxSeaLogo=window.__lxSeaLogo||{}); [].slice.call((root||document).querySelectorAll("img[data-lxneedlogo]")).forEach(function(im){   var id=im.getAttribute("data-lxneedlogo"); if(!id)return;   im.removeAttribute("data-lxneedlogo");   if(seen[id]===null)return;   if(seen[id]){ im.src=seen[id]; return; }   fetch("/lxapi/assetlogo?asset="+encodeURIComponent(id))     .then(function(r){ return r.ok?r.json():null; })     .then(function(j){ var u=j&&j.image; seen[id]=u||null; if(u)im.src=u; })     .catch(function(){ seen[id]=null; }); }); } function lxSeaCount(list){ if(!list||list.id!=="spAssetList")return; var c=document.getElementById("spAssetCount"); if(!c)return; c.textContent="("+list.querySelectorAll(".sp-row").length+")"; } function paint(list,html){ if(list.innerHTML!==html){ list.innerHTML=html; try{ lxSeaLogos(list); }catch(_){} try{ lxSeaCount(list); }catch(_){} } }
  ...
  // Our published token registry. MUST live at top level: first time round this block was inserted
  // INSIDE recFromRow(), which scoped lxSeaReg to that function -- so the call site below threw a
  // ReferenceError while mapping results and the search returned nothing at all. It also re-fetched on
  // every row. The syntax was valid, which is exactly why a parse check did not catch it.
  //
  // window.__lxTokenRegistry is baked into <head> at build time, so this is known on the first paint.
  // The fetch stays only as a refresh for a page left open across a deploy.
