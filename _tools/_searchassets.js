...
function txt(s){ var e=a.querySelector(s); return e?e.textContent.trim().replace(/\\s+/g," "):""; }
  ...
  function lxHlOne(code,iss){
    ...
    return fetch("https://api.stellar.expert/explorer/public/asset?search="+encodeURIComponent(iss)+"&limit=20")
  ...
  function lxSeaHolders(list){
    try{
      ...
      for(var i=0;i<rows.length;i++){
        (function(row){
          var a=lxSeaIdOf(row.getAttribute("href"));
          if(!a)return;                                  // pools and wallets carry no CODE-ISSUER
          var sb=row.querySelector(".sp-sub"); if(!sb)return;
          lxHlOne(a.code,a.issuer).then(function(n){
            if(n==null||!sb.parentNode)return;           // unresolved stays as it is, never a guess
            sb.textContent=a.code+" "+MID+" "+nfmt(n)+" holders";
          });
        })(rows[i]);
      }
    }catch(_){}
