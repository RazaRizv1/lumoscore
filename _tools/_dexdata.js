...
const SCRIPT = `<script id="lx-dexmain">(function(){
  ...
  function nativePrice(add){
    ...
    _first.forEach(function(a){ a.__lite=1; });
    MINTS_READY=Promise.all(_first.map(function(a){ return loadAssetLite(a); })).then(function(){
      var still=_first.filter(function(a){ return !(a.px>0); });
      if(still.length){ still.forEach(function(a){ a.__px=1; });
        Promise.all(still.map(function(a){ return loadAsset(a).catch(function(){}); })).then(function(){ nativeState=2; touch(); });
      } else { nativeState=2; touch(); }
    });
  }
  ...
  function priceVisible(list){
    ...
    gate.then(function(){ (function wave(i){ if(i>=need.length)return;
      Promise.all(need.slice(i,i+4).map(function(a){ return loadAsset(a).catch(function(){}); }))
  ...
  function j(u){
    return queued(function(){
      return fetchJ(u).catch(function(e){
        // one retry on the primary before switching hosts: a dropped connection is not a missing asset,
        // and the mirror does not carry every asset (ZERO/UPT 404 there).
        return fetchJ(u).catch(function(){
