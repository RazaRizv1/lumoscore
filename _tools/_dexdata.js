...
.dex-mint-stat .v{font-weight:700;font-size:14px;color:var(--text,#0e0e10);white-space:nowrap}
</style>`;
...
const SCRIPT = `<script id="lx-dexmain">(function(){
  ...
  function j(u){ return fetchJ(u).catch(function(e){ if(u.indexOf("horizon.stellar.org")>=0){ return fetchJ(u.replace("horizon.stellar.org","horizon.stellar.lobstr.co")); } throw e; }); }
  ...
  function applyHeroStats(){
    ...
    if(!box){ box=document.createElement("div"); box.className="lx-dxstats";   // 2x2 grid like the Pools .lx-hstats
      box.innerHTML='<div class="lx-dxstat" data-k="vol"><span class="v">\\u2014</span><span class="l">24h Volume</span></div>'
        ...
        +'<div class="lx-dxstat" data-k="mkts"><span class="v">'+ASSETS.length+'</span><span class="l">Markets</span></div>'
    ...
    var vol=0,liq=0; ASSETS.forEach(function(a){ if(a.vol!=null)vol+=a.vol*xlmUsd; if(a.tvlUsd!=null)liq+=a.tvlUsd; });
    var top=ASSETS.slice().filter(function(a){return a.vol!=null&&a.vol>0;}).sort(function(a,b){return (b.vol||0)-(a.vol||0);})[0];
  ...
  function loadAsset(a){
    ...
    calls.push(j(H+"/trade_aggregations?"+base+"&resolution=86400000&order=desc&limit=2").then(function(d){ var r=recs(d);
      if(r[0]){ a.px=+r[0].close||+r[0].avg||a.px; a.vol=+r[0].counter_volume||0; a.high=+r[0].high||0; a.low=+r[0].low||0; a.trades=+r[0].trade_count||0; }
    ...
    calls.push(j(H+"/accounts/"+a.issuer).then(function(acc){ a.domain=acc.home_domain||""; touch(); if(acc.home_domain)loadToml(a,acc.home_domain); }).catch(function(){ a.domain=""; }));
  ...
  function loadData(){
    ...
    Promise.all(ASSETS.map(function(a){ return loadAsset(a).catch(function(){}); })).then(function(){ window.__lxDEXloaded=1; try{window.__lxDEXassets=ASSETS;window.__lxDEXmovers=moverData;window.__lxDEXlogoCss=logoCss;}catch(_){} touch(); });
