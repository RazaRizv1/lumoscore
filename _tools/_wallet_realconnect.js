  ...
  return wcClient().then(function(client){
    return wcTimeout(client.connect({requiredNamespaces:{stellar:{methods:WC_METHODS,chains:[WC_CHAIN],events:[]}}}),20000,'WalletConnect did not respond \\u2014 try again').then(function(res){
      ...
      return loadMod('https://esm.sh/@walletconnect/modal@2').then(function(mm){
        var Modal=mm.WalletConnectModal||mm.default;var md=new Modal({projectId:WC_PROJECT_ID});
        if(res.uri)md.openModal({uri:res.uri});return md;
...
var A={
  ...
  stellar:{
    ...
    lobstr:function(){return loadMod('https://esm.sh/@lobstrco/signer-extension-api').then(function(m){
      ...
      if(getPk)return Promise.resolve(getPk()).then(function(pk){if(!pk)throw notInstalled('LOBSTR');return {address:pk,transport:'ext'};});
      // No extension. Without a Project ID WalletConnect cannot init, so keep the old, useful message
      // rather than replacing it with one about configuration the visitor cannot act on.
      // Order matters: WalletConnect is the better experience where it is configured, so prefer it.
      // Fall back to SEP-7 on mobile - no relay, no project id, and verified opening LOBSTR on a
      // real device. Desktop with neither has nothing left to try.
      if(WC_PROJECT_ID)return wcConnect(WC_LOBSTR).then(function(a){return {address:a,transport:'wc'};});
      if(lxSep7Supported())return lxSep7Connect().then(function(a){return {address:a,transport:'sep7'};});
