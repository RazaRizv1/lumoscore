    ...
    return wcTimeout(client.connect({requiredNamespaces:{stellar:{methods:WC_METHODS,chains:[WC_CHAIN],events:[]}}}),20000,'WalletConnect did not respond \\u2014 try again').then(function(res){
      ...
      return loadMod('https://esm.sh/@walletconnect/modal@2').then(function(mm){
        var Modal=mm.WalletConnectModal||mm.default;
          ...
          // Show ONLY LOBSTR. The WalletConnect registry is almost entirely EVM wallets that cannot sign
          // a Stellar transaction, so listing them invites a user to pick one and fail. Excluding ALL
          // and then recommending a single id is the documented way to pin the list.
          var md=new Modal({projectId:WC_PROJECT_ID,
...
function lxPostConnectHome(){try{var _p=location.pathname||'';
  ...
  // appeared it navigated on its own, so the "Continue" sitting under the address was decorative: the
  // page changed whether or not it was pressed, and anyone reading the address they had just connected
  // was cut off mid-glance.
  //
  // The design already wires that button (close(); if(home){goHome();}) but its "home" variable is only
  // set by its own demo flow -- on a real connection it stays null and Continue just closes. So the
  // destination is attached here instead, to the click rather than to a timer.
  // (No backticks in this comment: it sits inside a template literal, and a stray one ends the string.)
  //
  // If the button is not there for any reason, nothing happens: staying put is the safe failure, and
  // navigating on a timer is the exact behaviour being removed.
  var _done=q('.lxw-done');
  if(!_done||_done.__lxHome)return;
  _done.__lxHome=1;
