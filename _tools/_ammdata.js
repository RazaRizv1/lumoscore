...
const SCRIPT = `<script id="lx-ammdata">(function(){
  ...
  function lxPoolSearch(qRaw){
    var panel=q("#panelAll"); if(!panel)return;
    ...
    var cards=[].slice.call(panel.querySelectorAll(".pool-card:not(.lx-pscard)"));
    var lc=qq.toLowerCase(), hits=0;
    cards.forEach(function(c){
      if(!qq){ c.style.display=""; return; }
      var t=((c.querySelector(".pc-name")||{}).textContent||"")+" "+((c.querySelector(".pc-sub")||{}).textContent||"");
      var on=t.toLowerCase().indexOf(lc)>=0;
