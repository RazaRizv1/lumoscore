...
const SCRIPT = `<script id="lx-ammdata">(function(){
 ...
 var suf=/-dark\\./.test(location.pathname)?"-dark":(/-mobile\\./.test(location.pathname)?"-mobile":""); var base="lumoscore-amm-pool"+suf+".html"; if(fromDest&&String(fromDest).indexOf("amm-pool")>=0)base=String(fromDest).split("?")[0]; base=base.split("-light.html").join(".html"); if(["lumoscore-amm-pool.html","lumoscore-amm-pool-dark.html","lumoscore-amm-pool-mobile.html"].indexOf(base)<0)base="lumoscore-amm-pool"+suf+".html"; return base+"?pool="+hex; }
  ...
  function chDraw(){
    ...
    // The design sizes this svg per layout and sets the viewBox itself — 1000x280 on desktop, but 400x220
    // on the phone — while we always lay the series out in a 1000x280 space. So on mobile everything below
    // y=220 (the lower half of the line and the whole date row) fell outside the viewBox and was clipped:
    // the chart "cropped from the bottom". Pin the box to the space we actually draw in. No-op on desktop,
    // where the two already agree.
    try{ if(svg.getAttribute("viewBox")!=="0 0 "+W+" "+H){ svg.setAttribute("viewBox","0 0 "+W+" "+H); }
         if(svg.getAttribute("preserveAspectRatio")!=="none")svg.setAttribute("preserveAspectRatio","none"); }catch(_){}
  ...
  function txRowM(t){
    ...
    return '<div class="tx-item lx-txrow" data-txtype="'+kind+'">'
      ...
      +'<div class="tx-time">'+when+'</div></div>';
  }
  function pdTxMobile(){
