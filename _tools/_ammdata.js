...
const SCRIPT = `<script id="lx-ammdata">(function(){
  ...
  // put it at risk.
  var NETPP=25;
  var NET={page:1,q:"",rows:null,total:0,pages:1,busy:0,err:"",ranked:0,unpriceable:0,seq:0};
  ...
  function netLegs(p){
    ...
    var la={code:a.code,issuer:a.issuer,native:a.code==="XLM"&&!a.issuer,amount:+a.amount||0};
    var lb={code:b.code,issuer:b.issuer,native:b.code==="XLM"&&!b.issuer,amount:+b.amount||0};
    return [la,lb];
  }
  // Liquidity reads as the XLM leg where there is one (what the old rows showed), else the first leg.
  function netLiq(p){
    var L=netLegs(p), m=L[0].native?L[0]:(L[1].native?L[1]:L[0]);
    return qty(m.amount)+" "+esc(m.code);
  }
  // The detail page is built around an X/XLM pair. A pool with no XLM leg has no detail page, so it opens
  // on the explorer instead -- same rule myRow/myCard already apply to non-XLM positions.
  function netHref(p){
    var L=netLegs(p);
    return (L[0].native||L[1].native) ? detailUrl(p.id,null,null)
      : ("https://stellar.expert/explorer/public/liquidity-pool/"+p.id);
  }
  ...
  function netRow(p,i){
    ...
    return '<tr class="lx-ammrow lx-netrow" data-pool="'+p.id+'" data-href="'+netHref(p)+'" style="cursor:pointer">'+
      '<td class="idx">'+idx+'</td>'+
      '<td><div class="pair-cell">'+icoPairG(L[0],L[1])+'<div><div class="pair-name">'+esc(L[0].code)+' / '+esc(L[1].code)+'</div>'+
      '<div class="pair-sub">'+p.fee+'% fee \\u00b7 Stellar AMM</div></div></div></td>'+
      '<td><div>'+netLiq(p)+'</div><div class="pair-sub">'+usd(p.tvl)+'</div></td>'+
      volCell+
  ...
  function netCard(p,i){
    ...
    return '<a class="pool-card lx-ammcard lx-netcard" data-pool="'+p.id+'" href="'+netHref(p)+'">'+
      '<div class="pc-head">'+icoPairG(L[0],L[1],true)+
      '<div class="pc-info"><div class="pc-name">'+esc(L[0].code)+' / '+esc(L[1].code)+'</div>'+
      '<div class="pc-sub">'+p.fee+'% fee \\u00b7 Stellar AMM</div></div>'+
      '<div class="pc-idx">#'+idx+'</div></div>'+
      '<div class="pc-stats">'+
      '<div class="pc-stat"><div class="l">Liquidity</div><div class="v">'+netLiq(p)+'</div><div class="vs">'+usd(p.tvl)+'</div></div>'+
      '<div class="pc-stat"><div class="l">24h Vol</div>'+volCell+'</div>'+
  ...
  function paintNet(){
    if(!netActive())return;
    var rows=NET.rows;
    // Nothing has arrived yet AND nothing failed: leave the page alone rather than flashing an empty
    // table over the skeleton.
    if(!rows&&!NET.err)return;
    ...
    if(body){
      var sig="net|"+NET.page+"|"+NET.q+"|"+(rows?rows.length:-1)+"|"+NET.err;
    ...
    if(mob){
      var msig="net|"+NET.page+"|"+NET.q+"|"+(rows?rows.length:-1)+"|"+NET.err;
 ...
 var suf=/-dark\\./.test(location.pathname)?"-dark":(/-mobile\\./.test(location.pathname)?"-mobile":""); var base="lumoscore-amm-pool"+suf+".html"; if(fromDest&&String(fromDest).indexOf("amm-pool")>=0)base=String(fromDest).split("?")[0]; base=base.split("-light.html").join(".html"); if(["lumoscore-amm-pool.html","lumoscore-amm-pool-dark.html","lumoscore-amm-pool-mobile.html"].indexOf(base)<0)base="lumoscore-amm-pool"+suf+".html"; return base+"?pool="+hex; }
  ...
  function chState(){
    ...
    return {type:s.type,metric:__lxMetricSel||s.metric,range:__lxRangeSel||s.range};
  }
  ...
  function chDraw(){
    ...
    function yfmt(x){ x=+x||0; var a=Math.abs(x), sc=a>=1e6?1e6:(a>=1e3?1e3:1), sfx=sc===1e6?("M"+_u):(sc===1e3?("K"+_u):_u);
      var d=2; if(_step>0){ d=Math.max(2,Math.min(5,Math.ceil(-Math.log10(_step/sc))+1)); if(!isFinite(d))d=2; }
      return sc===1?((a>=1?num(x):x.toFixed(Math.max(2,d)))+_u):((x/sc).toFixed(d)+sfx); }
