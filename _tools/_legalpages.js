...
const STYLE = '<style id="lx-legal-css">'
  ...
  // ---- support form ----
  + '.lxsup{max-width:620px}'
  + '.lxsup-row{display:flex;flex-direction:column;gap:6px;margin-bottom:15px}'
  + '.lxsup-row label{font-size:13.5px;font-weight:700;color:var(--text)}'
  + '.lxsup-row .hint{font-size:12.5px;color:var(--text-muted,#8a8fa3);font-weight:400}'
  + '.lxsup input,.lxsup textarea{width:100%;padding:11px 13px;border-radius:10px;'
  + 'border:1px solid var(--border,#26262c);background:var(--surface,#131317);color:var(--text);'
  + 'font-family:inherit;font-size:15px;line-height:1.5}'
  + '.lxsup textarea{min-height:150px;resize:vertical}'
  + '.lxsup input:focus,.lxsup textarea:focus{outline:2px solid var(--accent,#ea6a2c);outline-offset:1px;'
  + 'border-color:transparent}'
  + '.lxsup-send{margin-top:4px;padding:12px 26px;border:0;border-radius:10px;cursor:pointer;'
  + 'background:var(--accent,#ea6a2c);color:#fff;font-family:inherit;font-size:15px;font-weight:700}'
  + '.lxsup-send:disabled{opacity:.55;cursor:default}'
  + '.lxsup-msg{margin-top:14px;font-size:14.5px;line-height:1.6;display:none}'
...
// ---- Support -------------------------------------------------------------------------------------
const SUPPORT = '<div class="lxlg">'
  + '<h1>Get in touch</h1>'
  + '<p class="lxlg-sub">Tell us what happened and we will reply by email. We usually answer within one '
  + 'business day.</p>'
  + '<form class="lxsup" id="lxSupForm" novalidate>'
  + '<div class="lxsup-row"><label for="lxsEmail">Your email <span class="hint">— so we can reply'
  + '</span></label><input id="lxsEmail" name="email" type="email" autocomplete="email" required '
  + 'placeholder="you@example.com"></div>'
  + '<div class="lxsup-row"><label for="lxsName">Name <span class="hint">— optional</span></label>'
  + '<input id="lxsName" name="name" type="text" autocomplete="name" placeholder="What to call you"></div>'
  + '<div class="lxsup-row"><label for="lxsSubject">Subject</label>'
  + '<input id="lxsSubject" name="subject" type="text" required placeholder="A one-line summary"></div>'
  + '<div class="lxsup-row"><label for="lxsMsg">Message</label>'
  + '<textarea id="lxsMsg" name="message" required '
  + 'placeholder="What were you trying to do, and what happened instead?"></textarea></div>'
  + '<div class="lxsup-row"><label for="lxsWallet">Wallet address <span class="hint">— optional, '
  + 'helps us look things up</span></label>'
  + '<input id="lxsWallet" name="wallet" type="text" placeholder="G..."></div>'
  + '<div class="lxsup-row"><label for="lxsTx">Transaction ID <span class="hint">— optional, if your '
  + 'question is about one</span></label><input id="lxsTx" name="txHash" type="text" '
  + 'placeholder="Transaction hash"></div>'
  + '<button type="submit" class="lxsup-send" id="lxsSend">Send message</button>'
  + '<p class="lxsup-msg" id="lxsMsgOut"></p>'
  + '</form>'
  + '<p class="lxlg-warn" style="margin-top:26px">We will never ask for your seed phrase or private keys. '
  + 'Anyone who does is trying to steal from you.</p>'
  + '</div>'
  ...
  + 'if(/^G[A-Z2-7]{55}$/.test(a)){ var w=document.getElementById("lxsWallet"); if(w&&!w.value)w.value=a; } }catch(_){}'
  ...
  + '.then(function(r){ if(!r.ok||!r.d||!r.d.ok){ throw new Error((r.d&&r.d.error)||"send failed"); }'
  ...
  + '.catch(function(){ say("That did not send. Email ' + SUPPORT_TO + ' directly and we will pick it up.","err"); })'
