...
// (LXLP_XLM removed — a relative asset path that 404'd on every clean route; its only consumer, the
//  "Paying from" avatar, now draws an identicon instead.)
...
function lxLpWireReview(){
  ...
  if(iconRow){ var mono=iconRow.querySelector(".v > div"); if(mono){ mono.style.cssText="width:36px;height:36px;border-radius:10px;overflow:hidden;display:block;background:var(--surface,#fff);border:1px solid var(--border,#e6e8ee)"; mono.innerHTML=lxLpImgTag(d.icon||LXLP_PH); }
    // ...and the MOCK filename baked beside it. "icon-slr.png · 142 KB" was printed to every launcher
    // whatever they uploaded, and to launchers who uploaded nothing — the thumbnail was swapped for the
    // real image while the caption still named a file that never existed. The real name is captured at
    // pick time (fi change -> __lxLpIconName) and round-trips in the draft. Only the trailing text span
    // is touched: the thumbnail and the "Uploaded" pill above it are correct and must survive. (No size
    // is shown — only the SHRUNK data URL is kept, so the original's byte count is no longer knowable.)
    var _iv=iconRow.querySelector(".v");
    var _ipill=_iv?_iv.querySelector(".pill"):null;
    var _iname=_iv?[].slice.call(_iv.children).filter(function(el){ return el.tagName==="SPAN" && !el.classList.contains("pill"); })[0]:null;
    if(_iv && !_iname){ _iname=document.createElement("span"); _iname.style.cssText="color:var(--text-soft)"; _iv.appendChild(_iname); }   // the phone layout ships no caption span
    if(_iname) _iname.textContent = d.icon ? String(d.iconName||"").trim() : "No icon uploaded";
    if(_ipill) _ipill.style.display = d.icon ? "" : "none";   // the pill reads "Uploaded" over a tick — a lie with no icon
  }
  // Paying from: an identicon of the connected address, as a fixed round avatar (the original box was a
  // shrinking rounded-square). This used to paint the Stellar logo from a RELATIVE asset path, which
  // 404'd on every clean route — see lxLpIdenticon.
  var pfReal=localStorage.getItem("lumos.address");
  var av=document.querySelector(".wallet-info .av"); if(av){ av.style.cssText="width:34px;height:34px;flex:0 0 34px;border-radius:50%;overflow:hidden;background:transparent;color:transparent;display:block;padding:0"; av.innerHTML=lxLpIdenticon(pfReal); }
  var pfAddr=document.querySelector(".wallet-info .addr"); if(pfAddr){ if(pfReal) pfAddr.textContent=lxLpShort(pfReal); pfAddr.style.whiteSpace="nowrap"; }
  ...
  lxLpSetRow(/Twitter|^X/i, d.twitter||"Not provided");
...
// Identicon for the "Paying from" avatar, drawn from the address itself. Replaces an <img> pointing at
// the RELATIVE path "assets/tokens/xlm.png": every launchpad page is served from a clean route
// (/launchpad/review), so that resolved to /launchpad/assets/... and 404'd — the broken image users saw.
// Inline SVG has no path to get wrong, differs per wallet, and (needing an svg child) is also what keeps
// the ticker-logo healer from repainting the box.
function lxLpIdenticon(addr){
  var s=String(addr||""), h=2166136261;
  for(var i=0;i<s.length;i++){ h^=s.charCodeAt(i); h=(h*16777619)>>>0; }          // FNV-1a
  var hue=h%360, r=h||1, cells="";
  function nxt(){ r^=r<<13; r>>>=0; r^=r>>>17; r^=r<<5; r>>>=0; return r; }       // xorshift32
  for(var y=0;y<5;y++){ for(var x=0;x<3;x++){ if(nxt()%2){                        // left half + centre...
    cells+='<rect x="'+x+'" y="'+y+'" width="1" height="1"/>';
    if(x<2) cells+='<rect x="'+(4-x)+'" y="'+y+'" width="1" height="1"/>';        // ...mirrored right
  } } }
  return '<svg viewBox="0 0 5 5" width="100%" height="100%" shape-rendering="crispEdges" '
    +'style="display:block" aria-hidden="true"><rect width="5" height="5" fill="hsl('+hue+',65%,90%)"/>'
    +'<g fill="hsl('+hue+',58%,42%)">'+cells+'</g></svg>';
}
