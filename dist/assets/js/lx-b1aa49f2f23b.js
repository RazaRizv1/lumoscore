  ...
  (function() {
    ...
    document.querySelectorAll('.type-card').forEach(card => {
      card.addEventListener('click', () => {
        ...
        // Also tick the radio so semantic state matches the visual one
  ...
  (function() {
    ...
    // Track whether icon was set by upload so we don't override it from ticker
    ...
    function handleFile(file) {
      ...
      reader.onload = function(e) {
        ...
        // Update the upload zone — replace icon-circle's SVG with the uploaded image
    ...
    // Base fees in APT (the displayed defaults).
    // We use APT as the authoritative number and derive USD via this rate.
    const BASE_CREATION_APT = 34.70;
    const BASE_LIQUIDITY_APT = 69.40;
    const BASE_NETWORK_APT = 69.40;
    // USD/APT rate derived from displayed defaults (173.50 APT ≈ $25 → ~$0.1441 per APT)
    const USD_PER_APT = 25 / 173.50;

    function fmtXlm(n) {
      // Use 2 decimals for small, comma for large
    ...
    const rowCreation = document.getElementById('costCreation');
    const rowLiquidity = document.getElementById('costLiquidity');
    const rowNetwork = document.getElementById('costNetwork');
    ...
    // Expose for use inside supply input handler above
