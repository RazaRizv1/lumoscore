...
(function() {
  ...
  // Per-chain circulating supply (used for market cap in tooltip)
  var stellarSupply = 618e6;
  var xrplSupply    = 552e6;

  // Defining a few realistic time-range presets. Each preset gives us how
  // many points we plot, how far back the window starts, what the x-axis
  // labels should read, and which seeds to use so the lines feel different
  // across ranges (rather than the same noisy 7D curve stretched out).
  // Volatility scales gently with range — longer windows = wider swings.
  ...
  function buildSeries(rng, base, vol, nPts) {
    var pts = [];
    var v = base;
    var drift = base * 0.0008; // small upward drift over the window
  ...
  function buildTimestamps(stepMs, nPts) {
    var ts = [];
    for (var i = 0; i < nPts; i++) {
      ts.push(endTs - (nPts - 1 - i) * stepMs);
    }
    return ts;
  }

  // Format a timestamp into the right axis label for a given range mode
  ...
  // Cached per-render state used by the hover handler.
  var state = {
  ...
  function fmtPrice(p) {
    // For very small values fall back to more decimals so movement is visible
  ...
  function render(rangeKey) {
    ...
    // y-axis ticks (5 levels) — format depends on magnitude
    var yTicks = '';
    for (var k = 0; k <= 4; k++) {
      var v = minV + (k / 4) * rangeV;
      var ty = MT + plotH - (k / 4) * plotH;
      yTicks += '<text x="' + (W - 6) + '" y="' + (ty + 4) + '" text-anchor="end" font-family="JetBrains Mono, monospace" font-size="10.5" fill="#6e6d78">' + fmtPrice(v) + '</text>';
    }
    ...
    for (var m = 0; m < nLabels; m++) {
      ...
      xLabels += '<text x="' + tx.toFixed(0) + '" y="' + (H - 4) + '" text-anchor="' + anchor + '" font-family="Plus Jakarta Sans, sans-serif" font-size="10.5" fill="#6e6d78">' + fmtAxis(timestamps[i], r.lblFmt) + '</text>';
    }
    ...
    wrap.innerHTML =
      '<svg id="priceChartSvg" viewBox="0 0 ' + W + ' ' + H + '" preserveAspectRatio="none" style="overflow:visible">' +
        '<defs>' +
          ...
          '<linearGradient id="gradXrpl" x1="0" x2="0" y1="0" y2="1">' +
            '<stop offset="0%" stop-color="#00aae4" stop-opacity="0.18"/>' +
            '<stop offset="100%" stop-color="#00aae4" stop-opacity="0"/>' +
          '</linearGradient>' +
        '</defs>' +
        ...
        '<circle id="chartDotAptos" r="4.5" fill="#ea6a2c" stroke="#0a0a0b" stroke-width="2" opacity="0"/>' +
      ...
      '<div id="chartTooltip" class="chart-tooltip" aria-hidden="true">' +
        ...
        '<div class="ct-row" style="display:none">' +
          '<span class="ct-dot" style="background:#00aae4"></span>' +
          '<span class="ct-name">LUMOS (Aptos)</span>' +
          '<span class="ct-spacer"></span>' +
  ...
  // Initial render based on which tab is active in the markup (defaults to 7D)
...
(function() {
  ...
  // -- Copy to clipboard (issuer addresses) --
