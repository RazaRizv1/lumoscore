...
function maxShares(dd){
  ...
  function hideCpFee(){
    ...
    for(var i=0;i<rows.length;i++){
      ...
      var t=(r.textContent||"").replace(/\s+/g," ").trim();
      // No  after "fee". The label and the value are adjacent elements with no whitespace between
      // them, so this row's textContent is literally "Trading fee0.5%" -- and  demands a boundary
